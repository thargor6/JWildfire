...
set of features (in comparison to the full JWildfire application); for example, there is no 3D-rendering.

=== Supported platform
GPU-rendering is currently only supported on Windows 10 and requires a CUDA-capable graphics card.

You will need a modern graphics card with plenty of memory to get the best use of this
engine.

For example: NVIDIA 1060 or better. This is also the minimum requirement supported from my side.
