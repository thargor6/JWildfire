...
====== _Shadow intensity_
Intensity of the shadow cast by this lightsource.
...
Mathematically spoken, this is more complex, but the basic is easy understand:
...
But a transformation which moves a point along a gentle wave might as valid as a transformation which moves
...
There are different names for nonlinear transformations:
 ...
 - *nonlinear transformation*: this is the prefered name
...
But this is intended only for exploration of some new ideas. Afte exploration they are included
...
Some of them really a lot, they can be thought as complete independant simulating programs.

The possibility of changing these parameters together with the superposition of different nonlinear transformations
together with the possibility to change parameters of affine transformations leads to an infinite number of
...
- *Supports external shapes*: a transformation which can be customized by using external resources like images, svg's or meshes.
...
The _Filter_ option consists of two drop-down lists, where you can choose up two _Variation profiles_:
 ...
 - the *second* drop-down list alternatively allows to select an *_inversed variation profile_*.
When you select an *_inversed variation profile_* in the *second* option, only variations are displayed which are
inside the first selected _Variation profile_, but not in the _inversed variation profile_.

[.thumb]
image::nonlinear_variations_filter.jpg[scaledwidth=50%]
...
[.thumb]
image::nonlinear_edit_variation_profiles.jpg[scaledwidth=75%]
...
All the other possiblities are also very interesting, but lead to much more subtle differences (in the most of the cases).
...
is the default for normal transforms. It is based on a gradient, which can be selected and edited in the _Gradient_ tab,
...
On each iteration, the point's color will be be interpolated between the current color and the target color.
...
*Distance* is especially effective for coloring flames made with a single non-blur transform, which are otherwise
quite difficult to color.

 - *Cyclic*: uses the grad1ient, but simply shifts the gradient index right the amount specified by Speed.
...
- *Opaque*: the contribution of the currently selected transformation is faded out by taking into account the_Opacity_ parameter.
...
weighting field. The numerical field specifies the magnitude, in the drop-down list you select the name of the paramter.
A value of 0 means no influence by a weighting field.

===== _Jitter amount_
Overall strength (amplitude) of the weighting field.
...
So each slice contains pixels with a depth, which are three-dimensional pixels, called _Voxels_.
