...
- your own gradients. They are located in a folder outside the JWildfire-installation-drawer,
  ...
  _Preferences_ by setting the property _tinaGradientPath_.
...
Using this generator you have both a gradient and
matching color curves.
...
You may set the default batch size for quick mutations in the _Preferences_ (property _tinaQuickMutationBatchSize_).
...
This is probably one of the most popular features of JWildire, because using this function
...
You can change this behaviour by editing the parameter _tinaRandomBatchRefreshType_ in
the _Preferences_.

The results of the random batch can be influenced by changing the following parameters
...
See the following sub-sections for more details to the former four parameters.
...
==== Rnd Symmetry
Sets the random-symmetry-generator involved in generating random flames.
