...
It does does nothing else, e.g. not to load the current flame into the Interactive Renderer. This
is intended.
...
Please note, they are still flames and not solid objects or planes, so all flames will overlap if you design
them that way.
...
Besides this users manual I'm planning to write another book which brings those two worlds (thery and practice) together.
...
e.g. to modify the currently edited flame and to refresh the preview.
...
When you download scripts from the internet, e.g. from https://www.jwfsanctuary.club, they usually come
as .zip-file.
...
otherwise you could uncomment another line of this pre-generated code. You can also play with the values, e.g. to replace
the *360* by some smaller value when the rotations are too large.
...
E.g., to increase the value to 32 GB, you should the *Xmx*-value in the _JWildfire.cfg_ like this:
...
You can increase the UI scale by changing the startup parameter *sun.java2d.uiScale*, e.g.
