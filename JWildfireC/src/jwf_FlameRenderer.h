...
struct FlameRenderer {
  ...
  void renderImage(SimpleImage *pImage, SimpleHDRImage *pHDRImage, SimpleHDRImage *pHDRIntensityMap) {
    ...
    if (pHDRImage != NULL) {
      ...
      if (hasBGColor) {
        ...
        for (int i = 0; i < pHDRImage->imageHeight; i++) {
          for (int j = 0; j < pHDRImage->imageWidth; j++) {
            ...
            if (rbgPoint->red < 0.0f) {
              ...
              float lum = (rbgPoint->red * 0.299f + rbgPoint->green * 0.588f + rbgPoint->blue * 0.113f);
              if (lum > maxLum) {
                maxLum = lum;
              }
              else if (lum < minLum) {
        ...
        if (setBG) {
          float weight = (float) (0.25f * (maxLum + 3.0f * minLum));
  ...
  RenderedFlame *renderFlame(RenderInfo *pRenderInfo) {
    ...
    if(threadCount==1) {
      ...
      thread.init(flame,flameTransformationContext,nSamples,flameView,raster,rasterWidth,rasterHeight,colorMap,paletteIdxScl,pRenderInfo->poolSize,pRenderInfo->bundleSize);
      thread.initState();
      thread.iterate();
    }
    else {
  ...
  void createColorMap() {
    ...
    paletteIdxScl = (float)flame->palette->getSize() - 1.0f;
  }
