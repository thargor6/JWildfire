...
class Hexnix3DFunc: public Variation {
public:
	Hexnix3DFunc() {
	  majp = 1.0;
	  scale = 0.25;
	  zlift = 0.0;
	  _3side = 0.67;
		initParameterNames(4, "majp", "scale", "zlift", "3side");
	}
    ...
    if (_fcycle > 5) {// Resets the cyclic counting
      _fcycle = 0;
      _rswtch = (int) JWF_FLOOR((JWF_FLOAT)(pContext->randGen->random() * 3.0)); //  Chooses new 6 or 3 nodes
    }
    if (_bcycle > 2) {
      _bcycle = 0;
      _rswtch = (int) JWF_FLOOR((JWF_FLOAT)(pContext->randGen->random() * 3.0)); //  Chooses new 6 or 3 nodes
    ...
    JWF_FLOAT smRotxTP = 0.0;
    JWF_FLOAT smRotyTP = 0.0;
    JWF_FLOAT smRotxFT = 0.0;
    JWF_FLOAT smRotyFT = 0.0;
    JWF_FLOAT gentleZ = 0.0;
    ...
    JWF_FLOAT boost = 0.0; //  Boost is the separation distance between the two planes
    int posNeg = 1;
    int loc60;
    int loc120;
    JWF_FLOAT scale3 = _3side;
    ...
    if (majplane == 0) {
      pVarTP->z += smooth * pAffineTP->z * scale * zlift; // single plate instructions
    }
    else if (majplane == 1 && majp < 0.0) {// Transition for reversing plates  because  majp is negative value
      if (majp < -1.0 && majp >= -2.0) {
    ...
    if (majplane == 2 && majp < 0.0) {// Begin the splitting operation, animation transition is done
      if (posNeg > 0) {//  The splitting operation positive side
        pVarTP->z += (smooth * (pAffineTP->z * scale * zlift + boost));
      }
      else {//  The splitting operation negative side
        pVarTP->z = (pVarTP->z - (2.0 * smooth * pVarTP->z)) + (smooth * posNeg * (pAffineTP->z * scale * zlift + boost));
      }
    }
    else {//  majp > 0.0       The splitting operation
      pVarTP->z += smooth * (pAffineTP->z * scale * zlift + (posNeg * boost));
    }

    if (_rswtch <= 1) {//  Occasion to build using 60 degree segments
      loc60 = (int) JWF_FLOOR((JWF_FLOAT)(pContext->randGen->random() * 6.0)); // random nodes selection
      ...
      loc120 = (int) JWF_FLOOR((JWF_FLOAT)(pContext->randGen->random() * 3.0)); // random nodes selection
    ...
    */
	...
	Hexnix3DFunc* makeCopy() {
		return new Hexnix3DFunc(*this);
	}

	virtual void init(FlameTransformationContext *pContext, XForm *pXForm, JWF_FLOAT pAmount) {
    /*  Set up two major angle systems */
    _rswtch = (int) JWF_FLOOR((JWF_FLOAT)(pContext->randGen->random() * 3.0)); //  Chooses 6 or 3 nodes
    JWF_FLOAT hlift = JWF_SIN(M_PI / 3.0); // sin(60)
    _fcycle = 0;
    _bcycle = 0;
    _seg60x[0] = 1.0;
    _seg60x[1] = 0.5;
    ...
    _seg120x[1] = JWF_COS(7.0 * M_PI / 6.0); // rotate 30 degrees relative to the hex structure.
    _seg120x[2] = JWF_COS(11.0 * M_PI / 6.0); //
...
private:
  ...
  JWF_FLOAT _3side; // scales the triangle within the hex - the 120 degree figure
