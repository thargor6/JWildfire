...
public:
	...
	void transform(FlameTransformationContext *pContext, XForm *pXForm, XYZPoint *pAffineTP, XYZPoint *pVarTP, JWF_FLOAT pAmount) {
		if (use_cos_x == 1) {
			pVarTP->x += pAmount * (pAffineTP->x + _dampingX * scalex * JWF_COS(pAffineTP->y * freqx)) * _dampingX;
		}
		else {
			pVarTP->x += pAmount * (pAffineTP->x + _dampingX * scalex * JWF_SIN(pAffineTP->y * freqx)) * _dampingX;
		}
		if (use_cos_y == 1) {
			pVarTP->y += pAmount * (pAffineTP->y + _dampingY * scaley * JWF_COS(pAffineTP->x * freqy)) * _dampingY;
		}
		else {
			pVarTP->y += pAmount * (pAffineTP->y + _dampingY * scaley * JWF_SIN(pAffineTP->x * freqy)) * _dampingY;
		}
	...
	Waves2WFFunc* makeCopy() {
		return new Waves2WFFunc(*this);
	}
