...
public:
    ...
    if (use_cos_x == 1) {
      pVarTP->x += pAmount * (pAffineTP->x + _dampingX * scalex * cosf(pAffineTP->y * freqx)) * _dampingX;
    }
    else {
      pVarTP->x += pAmount * (pAffineTP->x + _dampingX * scalex * sinf(pAffineTP->y * freqx)) * _dampingX;
    }
    if (use_cos_y == 1) {
      pVarTP->y += pAmount * (pAffineTP->y + _dampingY * scaley * cosf(pAffineTP->x * freqy)) * _dampingY;
    }
    else {
      pVarTP->y += pAmount * (pAffineTP->y + _dampingY * scaley * sinf(pAffineTP->x * freqy)) * _dampingY;
    }
    ...
    _dampingX = fabsf(dampx) < EPSILON ? 1.0 : expf(dampx);
    _dampingY = fabsf(dampy) < EPSILON ? 1.0 : expf(dampy);
	}
