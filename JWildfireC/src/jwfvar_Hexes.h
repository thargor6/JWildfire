...
public:
	...
	// Xh = (Xo + JWF_SQRT(3) * Yo) / (3 * l)
	static const JWF_FLOAT a_hex = 1.0 / 3.0;
	static const JWF_FLOAT b_hex = 1.7320508075688772935 / 3.0;
	// Now:  Xh = ( a_hex * Xo + b_hex * Yo ) / l;

	// Yh = (-Xo + JWF_SQRT(3) * Yo) / (3 * l)
	static const JWF_FLOAT c_hex = -1.0 / 3.0;
	static const JWF_FLOAT d_hex = 1.7320508075688772935 / 3.0;
	// Now:  Yh = ( c_hex * Xo + d_hex * Yo ) / l;

	// Xo = 3/2 * l * (Xh - Yh)
	static const JWF_FLOAT a_cart = 1.5;
	static const JWF_FLOAT b_cart = -1.5;
	// Now:  Xo = ( a_cart * Xh + b_cart * Yh ) * l;

	// Yo = JWF_SQRT(3)/2 * l * (Xh + Yh)
	static const JWF_FLOAT c_cart = 1.7320508075688772935 / 2.0;
	static const JWF_FLOAT d_cart = 1.7320508075688772935 / 2.0;
	// Now:  Yo = ( c_cart * Xh + d_cart * Yh ) * l;

	// centre gives centre co-ordinates either from cache,
	// or calculated from scratch if needed
	void cell_centre(int x, int y, JWF_FLOAT s, JWF_FLOAT V[2]) {
	...
	void transform(FlameTransformationContext *pContext, XForm *pXForm, XYZPoint *pAffineTP, XYZPoint *pVarTP, JWF_FLOAT pAmount) {
		...
		trgL = JWF_POW(L1 + 1e-100, power) * scale;
