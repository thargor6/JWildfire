...
public class CrobFunc extends VariationFunc implements SupportsGPU {
  ...
  public VariationFuncType[] getVariationTypes() {
    return new VariationFuncType[]{VariationFuncType.VARTYPE_CROP, VariationFuncType.VARTYPE_SUPPORTS_GPU};
  }
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "float top_border, bottom_border, left_border, right_border;\n"
        ...
        + "float tmp;\n"
        + "float crob_top = __crob_top;\n"
        + "float crob_bottom = __crob_bottom;\n"
        + "float crob_left = __crob_left;\n"
        + "float crob_right = __crob_right;\n"
        ...
        + "      crob_right = 1.0;\n"
        + "    }\n"
        + "    float directblur = __crob_directBlur;\n"
        + "    if(directblur<0.0) directblur = 0.0;\n"
        + "\n"
        + "    short blur = (__crob_blur != 0) ? 1 : 0;\n"
        ...
        + "    } else {\n"
        + "      top_border = crob_top + minInt_2 * __crob_ratioBlur;\n"
        + "      bottom_border = crob_bottom - minInt_2 * __crob_ratioBlur;\n"
        + "      left_border = crob_left + minInt_2 * __crob_ratioBlur;\n"
        + "      right_border = crob_right - minInt_2 * __crob_ratioBlur;\n"
        + "    }\n"
        ...
        + "            xTmp = crob_right - powf(RANDFLOAT(), directblur) * __crob_ratioBlur * minInt_2;\n"
        + "            gradTmp = (yTmp - y0c) / (xTmp - x0c);\n"
        + "          } while (gradTmp < -1.0);\n"
        + "\n"
        ...
        + "            yTmp = crob_top + powf(RANDFLOAT(), directblur) * __crob_ratioBlur * minInt_2;\n"
        + "            gradTmp = (yTmp - y0c) / (xTmp - x0c);\n"
        + "          } while ((gradTmp <= 0.0) && (gradTmp > -1.0));\n"
        + "\n"
        ...
        + "    __px += xTmp;\n"
        + "    __py += yTmp;\n"
        + (context.isPreserveZCoordinate() ? "__pz += __crob * __z;\n" : "");
  }
}
