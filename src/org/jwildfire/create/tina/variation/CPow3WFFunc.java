...
public class CPow3WFFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "float c;\n"
        ...
        + "  float inv_spread;\n"
        + "  float full_spread;\n"
        + "ang = 2.0 * PI / __cpow3_wf_divisor;\n"
        + "    c = __cpow3_wf_r * cosf(PI / 2 * __cpow3_wf_a) / __cpow3_wf_divisor;\n"
        + "    d = __cpow3_wf_r * sinf(PI / 2 * __cpow3_wf_a) / __cpow3_wf_divisor;\n"
        + "    half_c = c / 2;\n"
        + "    half_d = d / 2;\n"
        + "    inv_spread = 0.5 / __cpow3_wf_spread;\n"
        + "    full_spread = 2 * PI * __cpow3_wf_spread;\n"
        + " float ai = __theta;\n"
        + "    float n = RANDFLOAT() * __cpow3_wf_spread;\n"
        + "    if (__cpow3_wf_discrete_spread >= 1.0) {\n"
        + "      n = (int) n;\n"
        + "    }\n"
        + "    if (ai < 0) {\n"
        + "      n++;\n"
        ...
        + "    float ri = __cpow3_wf * expf(half_c * lnr2 - d * ai);\n"
        + "    float ang2 = c * ai * half_d * lnr2 * ang * (RANDFLOAT() * __cpow3_wf_spread2 + __cpow3_wf_offset2);\n"
        + "    __px += ri * cosf(ang2);\n"
        + "    __py += ri * sinf(ang2);\n"
        + "\n"
        + (context.isPreserveZCoordinate() ? "      __pz += __cpow3_wf * __z;\n" : "");
  }
}
