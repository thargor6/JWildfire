...
public class BTransformFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "    float tau, sigma;\n"
        ...
        + "    float sins, coss;\n"
        + "\n"
        + "    tau = 0.5 * (logf(sqrf(__x + 1.0) + __y*__y) - logf(sqrf(__x - 1.0) + __y*__y)) / __bTransform_power + __bTransform_move;\n"
        + "    sigma = PI - atan2f(__y, __x + 1.0) - atan2f(__y, 1.0 - __x) + __bTransform_rotate;\n"
        + "    sigma = sigma / __bTransform_power + (2.0f*PI) / __bTransform_power * floorf(RANDFLOAT() * __bTransform_power);\n"
        + "\n"
        + "    if (__x >= 0.0)\n"
        + "      tau += __bTransform_split;\n"
        + "    else\n"
        + "      tau -= __bTransform_split;\n"
        + "    sinht = sinhf(tau);\n"
        + "    cosht = coshf(tau);\n"
        ...
        + "    coss = cosf(sigma);\n"
        + "    temp = cosht - coss;\n"
        + "    __px += __bTransform * sinht / temp;\n"
        + "    __py += __bTransform * sins / temp;\n"
