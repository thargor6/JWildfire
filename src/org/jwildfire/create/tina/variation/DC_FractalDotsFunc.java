...
public class DC_FractalDotsFunc  extends DC_BaseFunc implements SupportsGPU {
	...
	public String getGPUCode(FlameTransformationContext context) {
	 return	 "float x,y;"
			...
			+"	    		float z=0.5;"
			+"	    		if( __dc_fractaldots_ColorOnly ==1)"
			+"	    		{"
			+"	    		  x=__x;"
			+"	    		  y=__y;"
			+"	    		}"
			...
			+"	    		color=dc_fractaldots_getRGBColor(uv,__dc_fractaldots_zoom,__dc_fractaldots_iterations,"
			+"                 __dc_fractaldots_dotsize,__dc_fractaldots_maxiterations,__dc_fractaldots_complexity,"
			+"                 __dc_fractaldots_pattern,__dc_fractaldots_spacing,__dc_fractaldots_rotate1,"
			+"                 __dc_fractaldots_rotate2);"
			+"	    		if( __dc_fractaldots_Gradient ==0 )"
			+"	    		{"
			+"	    		   __useRgb  = true;"
			+"	    		   __colorR  = color.x;"
			+"	    		   __colorG  = color.y;"
			...
			+"	    		   __colorA  = 1.0;"
			+"	    		}"
			+"	    		else if( __dc_fractaldots_Gradient ==1 )"
			+"	    		{"
			...
			+"	    		   __colorA  = 1.0;"
			+"	    		}"
			+"	    		else if( __dc_fractaldots_Gradient ==2 )"
			+"	    		{"
			+"	    		  int3 icolor=dbl2int(color);"
			+"	    		  float z=greyscale((float)icolor.x,(float)icolor.y,(float)icolor.z);"
			+"	    		  __pal=z;"
			+"	    		}"
			+"	    		__px+= __dc_fractaldots*x;"
			+"	    		__py+= __dc_fractaldots*y;"
			+"	    		float dz = z * __dc_fractaldots_scale_z + __dc_fractaldots_offset_z;"
			+"	    		if ( __dc_fractaldots_reset_z  == 1) {"
			+"	    		     __pz = dz;"
			+"	    		}"
			+"	    		else {"
			+"	    		   __pz += dz;"
