...
public class BWRandsFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return   "  float _g2;"  
    		...
    		+"  float _rfactor;"  
    		+"  int _petx, _pety;"
    		+"    float radius = 0.5 * ( __bwrands_cellsize  / (1.0 +  __bwrands_space  * __bwrands_space));"
    		+"    _g2 = sqrf( __bwrands_gain ) / radius + 1.0e-6;"
    		...
    		+"    _petx = fminf( __bwrands_minpetals ,  __bwrands_maxpetals );"
    		+"    _pety = fabsf( __bwrands_maxpetals  -  __bwrands_minpetals );"
    		+"    float Vx, Vy; "
    		+"    float Cx, Cy; "
    		+"    float Lx, Ly; "
    		+"    float r, theta, s, c;"
    		...
    		+"    Vx = __x;"
    		+"    Vy = __y;"
    		+"    if (fabsf( __bwrands_cellsize ) < 1.e-6f) {"
    		+"      __px += __bwrands * Vx;"
    		+"      __py += __bwrands * Vy;"
    		+"    }"
    		+"    else "
    		+"    {"
    		+"      int Ix = (int) floorf(Vx /  __bwrands_cellsize );"
    		+"      int Iy = (int) floorf(Vy /  __bwrands_cellsize );"
    		+"      Cx = (Ix + 0.5) *  __bwrands_cellsize ;"
    		+"      Cy = (Iy + 0.5) *  __bwrands_cellsize ;"
    		+"      int xx = Ix ^ 0xB641;"
    		+"      int yy = Iy ^ 0x9D81;"
    		+"      int xy = xx * yy +  __bwrands_seed ;"
    		+"      xx &= 0xFFFF;"
    		+"      yy &= 0xFFFF;"
    		+"      xy &= 0xFFFF;"
    		+"      int tt = bwrands_bytemix(xx, yy);"
    		...
    		+"      Ssz =  __bwrands_rmin  + Ssz * (1.0 -  __bwrands_rmin );"
    		+"      float Aan =  __bwrands_rrot  * (2.0f*PI) * ((float) yy) / 65535.0; "
    		+"      tt = bwrands_byteprimes(xx, yy);"
    		+"      yy = bwrands_byteprimes(yy, xx);"
    		+"      xx = tt;"
    		+"      float LoonieChance = -((float) xx) / 65535.0 +  __bwrands_loonie_chance ; "
    		+"      float PetalsChance = 0.0;"
    		+"      if (LoonieChance < 0)"
    		+"        PetalsChance = LoonieChance +  __bwrands_petals_chance ;"
    		+"      float NPetals;"
    		+"      if (_pety == 0)"
    		+"        NPetals = (float) _petx; "
    		+"      else"
    		+"        NPetals = (float) (_petx + (yy >> 3) % (1 + _pety)); "
    		+"      if (LoonieChance <= PetalsChance)"
    		+"        LoonieChance = -1.0; "
    		+"      Lx = Vx - Cx;"
    		+"      Ly = Vy - Cy;"
    		...
    		+"       } else if (PetalsChance > 0.0) "
    		+ "      {"
    		+"         flwr = NPetals / (2.0f*PI) * (PI + atan2f(Ly, Lx));"
    		+"         flwr = flwr - (int) (flwr);"
    		+"         flwr = fabsf(flwr - 0.5) * 2.0;"
    		+"         r = sqrtf(Lx * Lx + Ly * Ly);"
    		+"         PetalsChance = RANDFLOAT();"
    		+"         if (PetalsChance < .5 * (flwr + .5)) {"
    		+"           Lx *= (1.0 - r) * (flwr * 1.1); "
    		+"           Ly *= (1.0 - r) * (flwr * 1.1); "
    		+"         } else {"
    		+"           Vv2 = sqrtf(Vv2); "
    		+"           Lx *= (Vv2 - r * (1.0 - flwr)) / (r + 1.e-6f);"
    		+"           Ly *= (Vv2 - r * (1.0 - flwr)) / (r + 1.e-6f);"
    		+"         }"
    		+"         Vv2 = 1.0; "
    		+"       } else {"
    		+"         Lx *= _g2;"
    		...
    		+"         Vv2 = sqrtf(Ssz); "
    		+"       }"
    		+"       r = Vv2 * (Lx * Lx + Ly * Ly) / _r2; "
    		+"       theta =  __bwrands_inner_twist  * (1.0 - r) +  __bwrands_outer_twist  * r;"
    		+"       s = sinf(theta + Aan);"
    		+"       c = cosf(theta + Aan);"
    		+"       Vx = Cx + c * Lx + s * Ly;"
    		+"       Vy = Cy - s * Lx + c * Ly;"
            ...
            +"    }"
    		+"  }"
            + (context.isPreserveZCoordinate() ? "__pz += __bwrands * __z;" : "");
    
  }
  @Override
  public String getGPUFunctions(FlameTransformationContext context) {
	    return   "__device__ int  bwrands_bytemix (int a, int b) {"
	    		+"    return ((a & 0x5A5A) ^ (b & 0xA5A5));"
	    		+"  }"
	    		+"__device__ int  bwrands_bytexim (int a, int b) { "
	    		+"    return ((a & 0xAAAA) ^ (b & 0x5555));"
	    		+"  }"
	    		+"__device__ int  bwrands_byteshf (int a, int b) {"
	    		+"    return ((a << 8) ^ (b >> 8)) & 0xFFFF;"
	    		+"  }"
	    		+"__device__ int  bwrands_byteprimes (int a, int b) { "
	    		+"    return ((a * 857 - 4) & 0xFF00 ^ (-b * 977 + 8) & 0x00FF);"
	    		+"  }";
