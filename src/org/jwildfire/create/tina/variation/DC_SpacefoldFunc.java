	 ...
	 @Override
	  public String getGPUCode(FlameTransformationContext context) {
	    return   "float x,y;"
	    		...
	    		+"float z=0.5;"
	    		+"if( varpar->dc_spacefold_ColorOnly ==1)"
	    		+"{"
	    		+"  x=__x;"
	    		+"  y=__y;"
	    		+"}"
	    		...
	    		+"  y=RANDFLOAT()-0.5;"
	    		+"}"
	    		+"float2 uv=make_float2(x,y)*varpar->dc_spacefold_zoom;"
	    		+"color=dc_spacefold_getRGBColor(uv,varpar->dc_spacefold_time);"
	    		+"if( varpar->dc_spacefold_Gradient ==0 )"
	    		+"{"
	    		+"   __useRgb  = true;"
	    		+"   __colorR  = color.x;"
	    		+"   __colorG  = color.y;"
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( varpar->dc_spacefold_Gradient ==1 )"  
	    		+"{"
...
////use nearest palette color as the pixel color                
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( varpar->dc_spacefold_Gradient ==2 )"
	    		+"{"
	    		+"  int3 icolor=dbl2int(color);"
	    		+"  float z=greyscale((float)icolor.x,(float)icolor.y,(float)icolor.z);"
	    		+"  __pal=z;"
	    		+"}"
	    		+"__px+= varpar->dc_spacefold*x;"
	    		+"__py+= varpar->dc_spacefold*y;"
	    		+"float dz = z * varpar->dc_spacefold_scale_z + varpar->dc_spacefold_offset_z;"
	    		+"if ( varpar->dc_spacefold_reset_z  == 1) {"
	    		+"     __pz = dz;"
	    		+"}"
	    		+"else {"
	    		+"   __pz += dz;"
