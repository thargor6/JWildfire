...
public class CornersFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "  float xs = __x * __x;\n"
        ...
        + "\n"
        + "    float ex, ey;\n"
        + "    if (__corners_log_mode == 0) {\n"
        + "      ex = powf(xs, __corners_x_power + __corners_xy_power_add) * __corners_mult_x;\n"
        + "      ey = powf(ys, __corners_y_power + __corners_xy_power_add) * __corners_mult_y;\n"
        + "    } else {\n"
        + "      ex = powf(logf((xs * __corners_mult_x) + 3) / logf(__corners_log_base), __corners_x_power + 2.25 + __corners_xy_power_add) - 1.33;\n"
        + "      ey = powf(logf((ys * __corners_mult_y) + 3) / logf(__corners_log_base), __corners_y_power + 2.25 + __corners_xy_power_add) - 1.33;\n"
        + "    }\n"
        + "\n"
        + "\n"
        + "    if (__x > 0) {\n"
        + "      __px += __corners * ex + __corners_x;\n"
        + "    } else {\n"
        + "      __px += __corners * -ex - __corners_x;\n"
        + "    }\n"
        + "\n"
        + "    if (__y > 0) {\n"
        + "      __py += __corners * ey + __corners_y;\n"
        + "    } else {\n"
        + "      __py += __corners * -ey - __corners_y;\n"
        + "    }\n"
        + (context.isPreserveZCoordinate() ? "      __pz += __corners * __z;\n" : "");
  }
}
