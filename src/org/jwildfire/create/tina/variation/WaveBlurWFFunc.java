...
public class WaveBlurWFFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "float ang = RANDFLOAT() * 2.0f*PI;\n"
        ...
        + " float r;\n"
        + "int count = lroundf(varpar->waveblur_wf_count);\n "
        + " if (RANDFLOAT() < 0.5) {\n"
        + "   r = acosf(rnd) - (lroundf(count*RANDFLOAT()) + 1) * PI + varpar->waveblur_wf_phase;\n"
        + " } else {\n"
        + "   r = acosf(-rnd) - (lroundf(count*RANDFLOAT()) + 1) * PI + varpar->waveblur_wf_phase;\n"
        + " }\n"
        ...
        + "__px += r * c;\n"
        + "__py += r * s;\n"
        + "if (lroundf(varpar->waveblur_wf_direct_color) == 1) {\n"
