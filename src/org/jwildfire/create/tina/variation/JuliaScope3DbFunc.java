...
public class JuliaScope3DbFunc extends VariationFunc implements SupportsGPU {
    ...
    return "int absPower = lroundf(fabsf(__juliascope3Db_power));\n"
        + "float cPower = __juliascope3Db_dist / __juliascope3Db_power * 0.5;"
        + "  float z = __z / absPower + __juliascope3Db_warp;\n"
        + "  float r2d = __x * __x * __juliascope3Db_za + __y * __y * __juliascope3Db_zb;\n"
        + "  float rz = __juliascope3Db * __juliascope3Db_zamount * powf(r2d + z * z, cPower * __juliascope3Db_zdist);\n"
        + "\n"
        + "  int rnd = (int)(RANDFLOAT() * absPower);\n"
        + "  float a;\n"
        + "  if (rnd % 2 == 0)\n"
        + "   a = (2 * PI * rnd + atan2f(__y, __x)) / __juliascope3Db_power;\n"
        + "  else\n"
        + "   a = (2 * PI * rnd - atan2f(__y, __x)) / __juliascope3Db_power;\n"
        + "\n"
        + "  float r;\n"
        + "  if (lroundf(__juliascope3Db_type) != 0) {\n"
        + "   r = __juliascope3Db * powf(__x*__x + __y*__y + __z*__z, cPower);\n"
        + "  } else {\n"
        + "   r = __juliascope3Db * powf(__x*__x + __y*__y, cPower);\n"
        + "  }\n"
        + "  float cosa = cosf(a);\n"
        + "  float sina = sinf(a);\n"
        + "\n"
        + "  __px = __px + r * cosa;\n"
        + "  __py = __py + r * sina;\n"
        + "  if (lroundf(__juliascope3Db_mode) != 0) {\n"
        + "   float sincosa = sinf(__juliascope3Db_wave1) * cosf(__juliascope3Db_wave2);\n"
        + "   __pz += rz * z * sincosa;\n"
        + "  } else {\n"
        + "   __pz += rz * z;\n"
        + "  }\n";
