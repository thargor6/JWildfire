...
public class VoronFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "int i,j,l,K,M,M1,N,N1;\n"
        ...
        + "Rmin=20.f;\n"
        + "M=(int)floorf(__x/__voron_step);\n"
        + "N=(int)floorf(__y/__voron_step);\n"
        + "for (i=-1; i<2; i++)\n"
        + "{\n"
        + "    M1=M+i;\n"
        + "    for (j=-1; j<2; j++)\n"
        + "    {\n"
        + "        N1=N+j;\n"
        + "        dX = 19*M1+257*N1+__voron_xseed;\n"
        + "        n = (dX<<13) ^ dX;\n"
        + "        DiscretNoise = (( (n * (n * n * 15731 + 789221) + 1376312589) & 0x7fffffff)*AM);\n"
        + "        K=(1+(int)floorf(__voron_num*DiscretNoise));\n"
        + "        for (l=0; l<K; l++)\n"
        + "        {\n"
        + "            dX=l+64*M1+15*N1+__voron_xseed;\n"
        + "            n = (dX<<13) ^ dX;\n"
        + "            DiscretNoise = (( (n * (n * n * 15731 + 789221) + 1376312589) & 0x7fffffff)*AM);\n"
        + "            X=(DiscretNoise+M1)*__voron_step;\n"
        + "            dX=l+21*M1+33*N1+__voron_yseed;\n"
        + "            n = (dX<<13) ^ dX;\n"
        + "            DiscretNoise = (( (n * (n * n * 15731 + 789221) + 1376312589) & 0x7fffffff)*AM);\n"
        + "            Y=(DiscretNoise+N1)*__voron_step;\n"
        + "        \n"
        + "            OffsetX=__x-X;\n"
        + "            OffsetY=__y-Y;\n"
        + "        \n"
        ...
        + "    }\n"
        + "}\n"
        + "__px += __voron*(__voron_k*(__x-X0)+X0);\n"
        + "__py += __voron*(__voron_k*(__y-Y0)+Y0);\n"
        + (context.isPreserveZCoordinate() ? "__pz += __voron*__z;\n" : "");
  }
}
