	 ...
	 @Override
	  public String getGPUCode(FlameTransformationContext context) {
	    return   "float x,y;"
	    		...
	    		+"float z=0.5;"
	    		+"if( varpar->dc_warping_ColorOnly ==1)"
	    		+"{"
	    		+"  x=__x;"
	    		+"  y=__y;"
	    		+"}"
	    		...
	    		+"  y=RANDFLOAT()-0.5;"
	    		+"}"
	    		+"float2 uv=make_float2(x,y)*varpar->dc_warping_zoom + make_float2(varpar->dc_warping_x0,varpar->dc_warping_x0);"
	    		+"color=dc_warping_getRGBColor(uv);"
	    		+"if( varpar->dc_warping_Gradient ==0 )"
	    		+"{"
	    		+"   __useRgb  = true;"
	    		+"   __colorR  = color.x;"
	    		+"   __colorG  = color.y;"
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( varpar->dc_warping_Gradient ==1 )"  
	    		+"{"
...
////use nearest palette color as the pixel color                
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( varpar->dc_warping_Gradient ==2 )"
	    		+"{"
	    		+"  int3 icolor=dbl2int(color);"
	    		+"  float z=greyscale((float)icolor.x,(float)icolor.y,(float)icolor.z);"
	    		+"  __pal=z;"
	    		+"}"
	    		+"__px+= varpar->dc_warping*x;"
	    		+"__py+= varpar->dc_warping*y;"
	    		+"float dz = z * varpar->dc_warping_scale_z + varpar->dc_warping_offset_z;"
	    		+"if ( varpar->dc_warping_reset_z  == 1) {"
	    		+"     __pz = dz;"
	    		+"}"
	    		+"else {"
	    		+"   __pz += dz;"
