  ...
  public VariationFuncType[] getVariationTypes() {
	...
	public String getGPUCode(FlameTransformationContext context) {
		 return	  "    float _notch_bottom, _notch_top;"
				 +"    _notch_bottom =  __dc_perlin_select_centre  -  __dc_perlin_select_range ;"
				 +"    _notch_bottom = (_notch_bottom > 0.75) ? 0.75 : _notch_bottom;"
				 +"    _notch_bottom = (_notch_bottom < -2.0) ? -3.0 : _notch_bottom;"
				 +"    _notch_top =  __dc_perlin_select_centre  +  __dc_perlin_select_range ;"
				 +"    _notch_top = (_notch_top < -0.75) ? -0.75 : _notch_top;"
				 +"    _notch_top = (_notch_top > 3.0) ? 3.0 : _notch_top;"

				 +"    float V[3] ;"
				 +"    float Vx = 0.0, Vy = 0.0, Col, r, theta, s, c, p=1.0, e;"
				 +"    int t;"
				 +"    t = 0;"
				 +"    do {"
				 +"      "
				 ...
				 +"      } else {"
				 +"        "
				 +"        switch ( (int) __dc_perlin_shape ) {"
				 +"          case 0:"
				 +"            Vx = (1.0 + __dc_perlin_edge) * (RANDFLOAT() - 0.5);"
				 +"            Vy = (1.0 + __dc_perlin_edge) * (RANDFLOAT() - 0.5);"
				 +"            r = Vx * Vx > Vy * Vy ? Vx : Vy;"
				 +"            if (r > 1.0 - __dc_perlin_edge) {"
				 +"              e = 0.5 * (r - 1.0 + __dc_perlin_edge) / __dc_perlin_edge;"
				 +"            }"
				 +"            break;"
				 +"          case 1:"
				 +"            r = RANDFLOAT() + RANDFLOAT();"
				 +"            r = (r > 1.0) ? 2.0 - r : r;"
				 +"            r *= (1.0 + __dc_perlin_edge);"
				 +"            if (r > 1.0 - __dc_perlin_edge) {"
				 +"              e = 0.5 * (r - 1.0 + __dc_perlin_edge) / __dc_perlin_edge;"
				 +"            }"
				 ...
				 +"            break;"
				 +"          case 2:"
				 +"            r = (1.0 + __dc_perlin_edge) * RANDFLOAT();"
				 +"            if (r > 1.0 - __dc_perlin_edge) {"
				 +"              e = 0.5 * (r - 1.0 + __dc_perlin_edge) / __dc_perlin_edge;"
				 +"            }"
				 +"            theta = RANDFLOAT() * (2.0f*PI);"
				 ...
				 +"      }"
				 +"      "
				 +"      switch ( (int) __dc_perlin_map) {"
				 +"        case 0:"
				 +"          V[0] = __dc_perlin_scale * Vx;"
				 +"          V[1] = __dc_perlin_scale * Vy;"
				 +"          V[2] = __dc_perlin_scale * __dc_perlin_z;"
				 +"          break;"
				 +"        case 1:"
				 +"          r = 1.0 / (Vx * Vx + Vy * Vy + 1.e-6f);"
				 +"          V[0] = __dc_perlin_scale * Vx * r;"
				 +"          V[1] = __dc_perlin_scale * Vy * r;"
				 +"          V[2] = __dc_perlin_scale * __dc_perlin_z;"
				 +"          break;"
				 +"        case 2:"
				 +"          r = 1.0 / (Vx * Vx + Vy * Vy + 0.5);"
				 ...
				 +"          V[0] = __dc_perlin_scale * Vx;"
				 +"          V[1] = __dc_perlin_scale * Vy;"
				 +"          V[2] = __dc_perlin_scale * (r + __dc_perlin_z);"
				 +"          break;"
				 +"        case 5:"
				 +"          r = 0.25 - (Vx * Vx + Vy * Vy);"
				 +"          if (r < 0.0) {"
				 ...
				 +"          V[0] = __dc_perlin_scale * Vx;"
				 +"          V[1] = __dc_perlin_scale * Vy;"
				 +"          V[2] = __dc_perlin_scale * (2 * r + __dc_perlin_z);"
				 +"          break;"
				 +"      }"
				 +"      p = dc_perlin_perlinNoise3D(V, __dc_perlin_amps, __dc_perlin_freqs, (int) __dc_perlin_octaves);"
				 +"      "
				 +"      if (p > 0.0) {"
				 +"        e = p * (1.0 + e * e * 20.0) + 2.0 * e;"
				 +"      } else {"
				 ...
				 +"      }"
				 +"    }"
				 +"    while ((e < _notch_bottom || e > _notch_top) && t++ < __dc_perlin_select_bailout);"
				 +"    "
				 +"    Col = __dc_perlin_centre + __dc_perlin_range * p;"
				 +"    __pal = Col - floorf(Col);"
				 +"    __px += __dc_perlin * Vx;"
				 +"    __py += __dc_perlin * Vy;"
				  +(context.isPreserveZCoordinate() ? "__pz += __dc_perlin * __z;" : "");
		}

		 public String getGPUFunctions(FlameTransformationContext context) {
//			 return ""
		return   "__device__ float dc_perlin_simplexNoise3D(float V[]) {"
				+"       int p[2050] = {"
				...
				+"    float skewIn = (V[_x_] + V[_y_] + V[_z_]) * skewF3;"
				...
				+"__device__ float dc_perlin_perlinNoise3D(float V[], float aScale, float fScale, int octaves) {"
				+"    int _x_ = 0;"
				+"    int _y_ = 1;"
				+"    int _z_ = 2;"
				+"    float n = 0.0; "
