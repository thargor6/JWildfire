...
public class DC_FractColorFunc  extends DC_BaseFunc implements SupportsGPU {
	...
	public String getGPUCode(FlameTransformationContext context) {
		   return   "float x,y;"
		    		...
		    		+"float z=0.5;"
		    		+"if( __dc_fractcolor_ColorOnly ==1)"
		    		+"{"
		    		+"  x=__x;"
		    		+"  y=__y;"
		    		+"}"
		    		...
		    		+"  y=2.0*RANDFLOAT()-1.0;"
		    		+"}"
		    		+"float2 uv=make_float2(x,y)*__dc_fractcolor_zoom;"
		    		+"color=dc_fractcolor_getRGBColor(uv,__dc_fractcolor_xpar,__dc_fractcolor_ypar,__dc_fractcolor_time,__dc_fractcolor_iters);"
		    		+"if( __dc_fractcolor_Gradient ==0 )"
		    		+"{"
		    		+"   __useRgb  = true;"
		    		+"   __colorR  = color.x;"
		    		+"   __colorG  = color.y;"
		    		...
		    		+"   __colorA  = 1.0;"
		    		+"}"
		    		+"else if( __dc_fractcolor_Gradient ==1 )"
		    		+"{"
		    		...
		    		+"   __colorA  = 1.0;"
		    		+"}"
		    		+"else if( __dc_fractcolor_Gradient ==2 )"
		    		+"{"
		    		+"  int3 icolor=dbl2int(color);"
		    		+"  float z=greyscale((float)icolor.x,(float)icolor.y,(float)icolor.z);"
		    		+"  __pal=z;"
		    		+"}"
		    		+"__px+= __dc_fractcolor*x;"
		    		+"__py+= __dc_fractcolor*y;"
		    		+"float dz = z * __dc_fractcolor_scale_z + __dc_fractcolor_offset_z;"
		    		+"if ( __dc_fractcolor_reset_z  == 1) {"
		    		+"     __pz = dz;"
		    		+"}"
		    		+"else {"
		    		+"   __pz += dz;"
