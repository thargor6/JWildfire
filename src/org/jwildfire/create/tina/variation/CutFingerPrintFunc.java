	 ...
	 @Override
	  public String getGPUCode(FlameTransformationContext context) {
	    return  "		    float x,y,px_center,py_center;"
	    		...
	    		+"		      py_center=0.0f;"
	    		+"		    }"
	    		+"		    float2 uv=make_float2(x,y)*__cut_fingerprint_zoom;"
	    		+"		    color=cut_fingerprint_getRGBColor(uv,__cut_fingerprint_seed,__cut_fingerprint_width);"
	    		+"		    __doHide=false;"
	  ...
	  public String getGPUFunctions(FlameTransformationContext context) {
	    ...
	    return   "__device__ float2  cut_fingerprint_hash2 ( float2 p )"
	    		...
	    		+"__device__ float3  cut_fingerprint_getRGBColor (float2 uv, float seed, float width)"
	    		+"	{"
	    		+"	    float3 color=make_float3(0.,0.,0.);"
	    		+"	    float bounds = smoothstep(9.,10.,length(uv*( make_float2(0.7,0.5))));"
	    		+"	    float a=0.;"
	    		+"	    float2 h = make_float2(floorf(7.* seed), 0.);"
	    		+"	    for(int i=0; i<50; i++){"
	    		+"	        float s=sign(h.x);"
	    		+"	        h =  cut_fingerprint_hash2 (h)*(make_float2(15.,20.));"
	    		+"	    	a += s*atan2f(uv.x-h.x, uv.y-h.y);"
	    		+"	    }"
