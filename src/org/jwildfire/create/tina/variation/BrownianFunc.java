...
//	 System.out.println("<c,x,y : > < " + p.code +" , " + p.x +" , " + p.y +">");
 ...
 public  void midpoint(double x0, double y0, double x1, double y1, double var, int n) {
	    ...
	    double xmid = 0.5 * (x0 + x1) + gaussian(0, Math.sqrt(var));
	    double ymid = 0.5 * (y0 + y1) + gaussian(0, Math.sqrt(var));
 ...
 public void draw()
 {
     midpoint(0, 0, 0, 0, variation / Math.sqrt(2), level);
 }
...
//      canvas.draw();
  ...
  public DoublePoint2D pDot(double x1, double y1)
  {
  double xout = 0, yout = 0;
  double xoffset=0, yoffset=0;
  // draw endpoints
