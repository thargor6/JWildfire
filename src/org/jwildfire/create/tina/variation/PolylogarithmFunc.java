...
public class PolylogarithmFunc extends VariationFunc implements SupportsGPU {
...
//      return;
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return   "    float N =  __polylogarithm_n;"
    		+"    float HSTerm = polylogarithm_HarmonicS((int) N - 1);"
    		
//    		+"    Riem = new RiemannInt();"

    		+"    float vv = __polylogarithm;"
    		+"    Complex z;"
    		+ "   Complex_Init(&z,__x, __y);"
    		+"    Complex_Pow(&z, __polylogarithm_zpow );"
 // z.Save();
    		+"    Complex z_save;"
    		...
    		+"        for (int i = 1; i < 20; i++) {"
    		+"          Complex t0;"
    		+"          Complex_Init(&t0,powf(i,N),0.0);"
    		+"          Complex_Copy(&T, &t0);"
//          +"          T.DivR(z);"
//    		+"          polylogarithm_DivR(&T,&z);"
			+"          Complex_DivR(&T,&z);"
    		+"          Complex_Add(&LiN,&T);"
//    		+"          z.NextPow();"
...
//    		+"        z.Save();"    		
    		...
    		+"        for (int i = 0; i < 20; i++) {"
   		    +"          double zee = polylogarithm_Riemann_Z((int) N - i);"
    		+"          if (zee != 0.0) {"
    		+"            Complex_Copy(&T,&z);"
...
//    		+"        z.Neg();"
    		...
    		+"        Complex_Copy(&T,&z);"
    		+"        Complex_Copy(&z,&zPl1);"
    		+"        Complex_Scale(&z,  (1.0f / polylogarithm_factorial((int) N - 1)) );"
    		+"        Complex_Mul(&z,&T);"
    		+"        Complex_Add(&LiN,&z);"  
    		+"        __px += vv * LiN.re;"
    		+"        __py += vv * LiN.im;"
            ...
            + (context.isPreserveZCoordinate() ? "__pz += __polylogarithm * __z;" : "");
  }
  @Override
  public String getGPUFunctions(FlameTransformationContext context) {
	    return   " __device__  double  polylogarithm_Riemann_Z (int N) {"
	    		+"      double RzNegOdd[]  = {-0.08333333333333333, "
	    		...
	    		+"            -1.827075266281457e+210,"
	    		+"            1.823538632259567e+213};"
	    		+"    double RzPosInt[] = {-0.50000000000000000000000000000, "
	    		+"            0.0, "
	    		+"            1.64493406684822643647241516665, "
	    		+"            1.20205690315959428539973816151,"
	    		+"            1.08232323371113819151600369654,"
	            ...
	            +"__device__ float  polylogarithm_factorial (int n) { "
	    		+"    int i;"
	    		+"    float prod=1.0;"
	    		+"    if (n != 0) {"
	    		+"       for (i = 2; i <= n; i++)"
	            ...
	            +"  __device__ void polylogarithm_DivR(Complex *T,Complex *zz) {"
	            ...
	            +"__device__ float  polylogarithm_HarmonicS (int N) { "
	    		+"    if (N < 1)"
	    		+"      return 0.;"
	    		+"    float Hs = 0.0;"
	    		+"    int i = 1;"
