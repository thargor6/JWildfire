...
public class VogelFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "float M_PHI = 1.61803398874989484820f;\n"
        ...
        + "float sina, cosa;\n"
        + "sincosf(a, &sina, &cosa);\n"
        + "float r = varpar->vogel * (__r + sqrtf(i));\n"
        + "__px += r * (cosa + (varpar->vogel_scale * __x));\n"
        + "__py += r * (sina + (varpar->vogel_scale * __y));\n"
        + (context.isPreserveZCoordinate() ? "__pz += varpar->vogel * __z;\n" : "");
  }
}
