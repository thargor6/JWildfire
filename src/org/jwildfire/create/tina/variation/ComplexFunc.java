...
public class ComplexFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "    float x = 0.0, y = 0.0;\n"
        + "\n"
        + "    if (varpar->complex_cospow != 0) {\n"
        + "      float cossin = sinf(__x * varpar->complex_cosx1);\n"
        + "      float coscos = cosf(__x * varpar->complex_cosx2);\n"
        + "      float cossinh = sinhf(__y * varpar->complex_cosy1);\n"
        + "      float coscosh = coshf(__y * varpar->complex_cosy2);\n"
        + "      x += varpar->complex_cospow * coscos * coscosh;\n"
        + "      y -= varpar->complex_cospow * cossin * cossinh;\n"
        + "    }\n"
        + "\n"
        + "    if (varpar->complex_coshpow != 0) {\n"
        + "      float coshsin = sinf(__y * varpar->complex_coshy1);\n"
        + "      float coshcos = cosf(__y * varpar->complex_coshy2);\n"
        + "      float coshsinh = sinhf(__x * varpar->complex_coshx1);\n"
        + "      float coshcosh = coshf(__x * varpar->complex_coshx2);\n"
        + "      x += varpar->complex_coshpow * coshcosh * coshcos;\n"
        + "      y += varpar->complex_coshpow * coshsinh * coshsin;\n"
        + "    }\n"
        + "    if (varpar->complex_cotpow != 0) {\n"
        + "      float cotsin = sinf(varpar->complex_cotx1 * __x);\n"
        + "      float cotcos = cosf(varpar->complex_cotx2 * __x);\n"
        + "      float cotsinh = sinhf(varpar->complex_coty1 * __y);\n"
        + "      float cotcosh = coshf(varpar->complex_coty2 * __y);\n"
        + "      float cotden = 1.0 / (cotcosh - cotcos);\n"
        + "      x += varpar->complex_cotpow * cotden * cotsin;\n"
        + "      y -= varpar->complex_cotpow * cotden * cotsinh;\n"
        + "    }\n"
        + "    if (varpar->complex_cothpow != 0) {\n"
        + "      float cothsin = sinf(varpar->complex_cothy1 * __y);\n"
        + "      float cothcos = cosf(varpar->complex_cothy2 * __y);\n"
        + "      float cothsinh = sinhf(varpar->complex_cothx1 * __x);\n"
        + "      float cothcosh = coshf(varpar->complex_cothx2 * __x);\n"
        + "      float cothd = (cothcosh - cothcos);\n"
        + "      if (cothd != 0) {\n"
        + "        float cothden = 1.0 / cothd;\n"
        + "        x += varpar->complex_cothpow * cothden * cothsinh;\n"
        + "        y += varpar->complex_cothpow * cothden * cothsin;\n"
        + "      }\n"
        + "    }\n"
        + "    if (varpar->complex_cscpow != 0) {\n"
        + "      float cscsin = sinf(__x * varpar->complex_cscx1);\n"
        + "      float csccos = cosf(__x * varpar->complex_cscx2);\n"
        + "      float cscsinh = sinhf(__y * varpar->complex_cscy1);\n"
        + "      float csccosh = coshf(__y * varpar->complex_cscy2);\n"
        + "      float cscd = (coshf(2.0 * __y) - cosf(2.0 * __x));\n"
        + "      if (cscd != 0) {\n"
        + "        float cscden = 2.0 / cscd;\n"
        + "        x += varpar->complex_cscpow * cscden * cscsin * csccosh;\n"
        + "        y -= varpar->complex_cscpow * cscden * csccos * cscsinh;\n"
        + "      }\n"
        + "\n"
        + "    }\n"
        + "\n"
        + "    if (varpar->complex_cschpow != 0) {\n"
        + "      float cschsin = sinf(__y * varpar->complex_cschy1);\n"
        + "      float cschcos = cosf(__y * varpar->complex_cschy2);\n"
        + "      float cschsinh = sinhf(__x * varpar->complex_cschx1);\n"
        + "      float cschcosh = coshf(__x * varpar->complex_cschx2);\n"
        + "      float cschd = (coshf(2.0 * __x) - cosf(2.0 * __y));\n"
        + "      if (cschd != 0) {\n"
        + "        float cschden = 2.0 / cschd;\n"
        + "        x += varpar->complex_cschpow * cschden * cschsinh * cschcos;\n"
        + "        y -= varpar->complex_cschpow * cschden * cschcosh * cschsin;\n"
        + "      }\n"
        + "    }\n"
        + "\n"
        + "    if (varpar->complex_exppow != 0) {\n"
        + "      float expe = expf(__x * varpar->complex_expx1);\n"
        + "      float expsin = sinf(__y * varpar->complex_expy1);\n"
        + "      float expcos = cosf(__y * varpar->complex_expy2);\n"
        + "      x += varpar->complex_exppow * expe * expcos;\n"
        + "      y += varpar->complex_exppow * expe * expsin;\n"
        + "    }\n"
        + "    if (varpar->complex_secpow != 0) {\n"
        + "      float secsin = sinf(__x * varpar->complex_secx1);\n"
        + "      float seccos = cosf(__x * varpar->complex_secx2);\n"
        + "      float secsinh = sinhf(__y * varpar->complex_secy1);\n"
        + "      float seccosh = coshf(__y * varpar->complex_secy2);\n"
        + "      float secd = (cosf(2.0 * __x) + coshf(2.0 * __y));\n"
        + "      if (secd != 0) {\n"
        + "        float secden = 2.0 / secd;\n"
        + "        x += varpar->complex_secpow * secden * seccos * seccosh;\n"
        + "        y += varpar->complex_secpow * secden * secsin * secsinh;\n"
        + "      }\n"
        + "    }\n"
        + "\n"
        + "    if (varpar->complex_sechpow != 0) {\n"
        + "      float sechsinh = sinf(__y * varpar->complex_sechy1);\n"
        + "      float sechcosh = cosf(__y * varpar->complex_sechy2);\n"
        + "      float sechsin = sinhf(__x * varpar->complex_sechx1);\n"
        + "      float sechcos = coshf(__x * varpar->complex_sechx2);\n"
        + "      float sechd = (cosf(2.0 * __y) + coshf(2.0 * __x));\n"
        + "      if (sechd != 0) {\n"
        + "        float sechden = 2.0 / sechd;\n"
        + "        x += varpar->complex_sechpow * sechden * sechcos * sechcosh;\n"
        + "        y -= varpar->complex_sechpow * sechden * sechsin * sechsinh;\n"
        + "      }\n"
        + "    }\n"
        + "\n"
        + "    if (varpar->complex_tanpow != 0) {\n"
        + "      float tansin = sinf(varpar->complex_tanx1 * __x);\n"
        + "      float tancos = cosf(varpar->complex_tanx2 * __x);\n"
        + "      float tansinh = sinhf(varpar->complex_tany1 * __y);\n"
        + "      float tancosh = coshf(varpar->complex_tany2 * __y);\n"
        + "      float tand = (tancos + tancosh);\n"
        + "      if (tand != 0) {\n"
        + "        float tanden = 1.0 / tand;\n"
        + "        x += varpar->complex_tanpow * tanden * tansin;\n"
        + "        y += varpar->complex_tanpow * tanden * tansinh;\n"
        + "      }\n"
        + "    }\n"
        + "    if (varpar->complex_tanhpow != 0) {\n"
        + "      float tanhsin = sinf(__y * varpar->complex_tanhy1);\n"
        + "      float tanhcos = cosf(__y * varpar->complex_tanhy2);\n"
        + "      float tanhsinh = sinhf(__x * varpar->complex_tanhx1);\n"
        + "      float tanhcosh = coshf(__x * varpar->complex_tanhx2);\n"
        + "      float tanhd = (tanhcos + tanhcosh);\n"
        + "      if (tanhd != 0) {\n"
        + "        float tanhden = 1.0 / tanhd;\n"
        + "        x += varpar->complex_tanhpow * tanhden * tanhsinh;\n"
        + "        y += varpar->complex_tanhpow * tanhden * tanhsin;\n"
        + "      }\n"
        + "    }\n"
        + "    if (varpar->complex_sinpow != 0) {\n"
        + "      float sinsin = sinf(__x * varpar->complex_sinx1);\n"
        + "      float sincos = cosf(__x * varpar->complex_sinx2);\n"
        + "      float sinsinh = sinhf(__y * varpar->complex_siny1);\n"
        + "      float sincosh = coshf(__y * varpar->complex_siny2);\n"
        + "      x += varpar->complex_sinpow * sinsin * sincosh;\n"
        + "      y += varpar->complex_sinpow * sincos * sinsinh;\n"
        + "    }\n"
        + "    if (varpar->complex_sinhpow != 0) {\n"
        + "      float sinhsin = sinf(__y * varpar->complex_sinhy1);\n"
        + "      float sinhcos = cosf(__y * varpar->complex_sinhy2);\n"
        + "      float sinhsinh = sinhf(__x * varpar->complex_sinhx1);\n"
        + "      float sinhcosh = coshf(__x * varpar->complex_sinhx2);\n"
        + "      x += varpar->complex_sinhpow * sinhsinh * sinhcos;\n"
        + "      y += varpar->complex_sinhpow * sinhcosh * sinhsin;\n"
        + "    }\n"
        + "    __px += varpar->complex * x;\n"
        + "    __py += varpar->complex * y;\n"
        + "\n"
        + (context.isPreserveZCoordinate() ? "      __pz += varpar->complex * __z;\n" : "");
  }
}
