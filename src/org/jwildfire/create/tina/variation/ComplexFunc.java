...
public class ComplexFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "    float x = 0.0, y = 0.0;\n"
        + "\n"
        + "    if (__complex_cospow != 0) {\n"
        + "      float cossin = sinf(__x * __complex_cosx1);\n"
        + "      float coscos = cosf(__x * __complex_cosx2);\n"
        + "      float cossinh = sinhf(__y * __complex_cosy1);\n"
        + "      float coscosh = coshf(__y * __complex_cosy2);\n"
        + "      x += __complex_cospow * coscos * coscosh;\n"
        + "      y -= __complex_cospow * cossin * cossinh;\n"
        + "    }\n"
        + "\n"
        + "    if (__complex_coshpow != 0) {\n"
        + "      float coshsin = sinf(__y * __complex_coshy1);\n"
        + "      float coshcos = cosf(__y * __complex_coshy2);\n"
        + "      float coshsinh = sinhf(__x * __complex_coshx1);\n"
        + "      float coshcosh = coshf(__x * __complex_coshx2);\n"
        + "      x += __complex_coshpow * coshcosh * coshcos;\n"
        + "      y += __complex_coshpow * coshsinh * coshsin;\n"
        + "    }\n"
        + "    if (__complex_cotpow != 0) {\n"
        + "      float cotsin = sinf(__complex_cotx1 * __x);\n"
        + "      float cotcos = cosf(__complex_cotx2 * __x);\n"
        + "      float cotsinh = sinhf(__complex_coty1 * __y);\n"
        + "      float cotcosh = coshf(__complex_coty2 * __y);\n"
        + "      float cotden = 1.0 / (cotcosh - cotcos);\n"
        + "      x += __complex_cotpow * cotden * cotsin;\n"
        + "      y -= __complex_cotpow * cotden * cotsinh;\n"
        + "    }\n"
        + "    if (__complex_cothpow != 0) {\n"
        + "      float cothsin = sinf(__complex_cothy1 * __y);\n"
        + "      float cothcos = cosf(__complex_cothy2 * __y);\n"
        + "      float cothsinh = sinhf(__complex_cothx1 * __x);\n"
        + "      float cothcosh = coshf(__complex_cothx2 * __x);\n"
        + "      float cothd = (cothcosh - cothcos);\n"
        + "      if (cothd != 0) {\n"
        + "        float cothden = 1.0 / cothd;\n"
        + "        x += __complex_cothpow * cothden * cothsinh;\n"
        + "        y += __complex_cothpow * cothden * cothsin;\n"
        + "      }\n"
        + "    }\n"
        + "    if (__complex_cscpow != 0) {\n"
        + "      float cscsin = sinf(__x * __complex_cscx1);\n"
        + "      float csccos = cosf(__x * __complex_cscx2);\n"
        + "      float cscsinh = sinhf(__y * __complex_cscy1);\n"
        + "      float csccosh = coshf(__y * __complex_cscy2);\n"
        + "      float cscd = (coshf(2.0 * __y) - cosf(2.0 * __x));\n"
        + "      if (cscd != 0) {\n"
        + "        float cscden = 2.0 / cscd;\n"
        + "        x += __complex_cscpow * cscden * cscsin * csccosh;\n"
        + "        y -= __complex_cscpow * cscden * csccos * cscsinh;\n"
        + "      }\n"
        + "\n"
        + "    }\n"
        + "\n"
        + "    if (__complex_cschpow != 0) {\n"
        + "      float cschsin = sinf(__y * __complex_cschy1);\n"
        + "      float cschcos = cosf(__y * __complex_cschy2);\n"
        + "      float cschsinh = sinhf(__x * __complex_cschx1);\n"
        + "      float cschcosh = coshf(__x * __complex_cschx2);\n"
        + "      float cschd = (coshf(2.0 * __x) - cosf(2.0 * __y));\n"
        + "      if (cschd != 0) {\n"
        + "        float cschden = 2.0 / cschd;\n"
        + "        x += __complex_cschpow * cschden * cschsinh * cschcos;\n"
        + "        y -= __complex_cschpow * cschden * cschcosh * cschsin;\n"
        + "      }\n"
        + "    }\n"
        + "\n"
        + "    if (__complex_exppow != 0) {\n"
        + "      float expe = expf(__x * __complex_expx1);\n"
        + "      float expsin = sinf(__y * __complex_expy1);\n"
        + "      float expcos = cosf(__y * __complex_expy2);\n"
        + "      x += __complex_exppow * expe * expcos;\n"
        + "      y += __complex_exppow * expe * expsin;\n"
        + "    }\n"
        + "    if (__complex_secpow != 0) {\n"
        + "      float secsin = sinf(__x * __complex_secx1);\n"
        + "      float seccos = cosf(__x * __complex_secx2);\n"
        + "      float secsinh = sinhf(__y * __complex_secy1);\n"
        + "      float seccosh = coshf(__y * __complex_secy2);\n"
        + "      float secd = (cosf(2.0 * __x) + coshf(2.0 * __y));\n"
        + "      if (secd != 0) {\n"
        + "        float secden = 2.0 / secd;\n"
        + "        x += __complex_secpow * secden * seccos * seccosh;\n"
        + "        y += __complex_secpow * secden * secsin * secsinh;\n"
        + "      }\n"
        + "    }\n"
        + "\n"
        + "    if (__complex_sechpow != 0) {\n"
        + "      float sechsinh = sinf(__y * __complex_sechy1);\n"
        + "      float sechcosh = cosf(__y * __complex_sechy2);\n"
        + "      float sechsin = sinhf(__x * __complex_sechx1);\n"
        + "      float sechcos = coshf(__x * __complex_sechx2);\n"
        + "      float sechd = (cosf(2.0 * __y) + coshf(2.0 * __x));\n"
        + "      if (sechd != 0) {\n"
        + "        float sechden = 2.0 / sechd;\n"
        + "        x += __complex_sechpow * sechden * sechcos * sechcosh;\n"
        + "        y -= __complex_sechpow * sechden * sechsin * sechsinh;\n"
        + "      }\n"
        + "    }\n"
        + "\n"
        + "    if (__complex_tanpow != 0) {\n"
        + "      float tansin = sinf(__complex_tanx1 * __x);\n"
        + "      float tancos = cosf(__complex_tanx2 * __x);\n"
        + "      float tansinh = sinhf(__complex_tany1 * __y);\n"
        + "      float tancosh = coshf(__complex_tany2 * __y);\n"
        + "      float tand = (tancos + tancosh);\n"
        + "      if (tand != 0) {\n"
        + "        float tanden = 1.0 / tand;\n"
        + "        x += __complex_tanpow * tanden * tansin;\n"
        + "        y += __complex_tanpow * tanden * tansinh;\n"
        + "      }\n"
        + "    }\n"
        + "    if (__complex_tanhpow != 0) {\n"
        + "      float tanhsin = sinf(__y * __complex_tanhy1);\n"
        + "      float tanhcos = cosf(__y * __complex_tanhy2);\n"
        + "      float tanhsinh = sinhf(__x * __complex_tanhx1);\n"
        + "      float tanhcosh = coshf(__x * __complex_tanhx2);\n"
        + "      float tanhd = (tanhcos + tanhcosh);\n"
        + "      if (tanhd != 0) {\n"
        + "        float tanhden = 1.0 / tanhd;\n"
        + "        x += __complex_tanhpow * tanhden * tanhsinh;\n"
        + "        y += __complex_tanhpow * tanhden * tanhsin;\n"
        + "      }\n"
        + "    }\n"
        + "    if (__complex_sinpow != 0) {\n"
        + "      float sinsin = sinf(__x * __complex_sinx1);\n"
        + "      float sincos = cosf(__x * __complex_sinx2);\n"
        + "      float sinsinh = sinhf(__y * __complex_siny1);\n"
        + "      float sincosh = coshf(__y * __complex_siny2);\n"
        + "      x += __complex_sinpow * sinsin * sincosh;\n"
        + "      y += __complex_sinpow * sincos * sinsinh;\n"
        + "    }\n"
        + "    if (__complex_sinhpow != 0) {\n"
        + "      float sinhsin = sinf(__y * __complex_sinhy1);\n"
        + "      float sinhcos = cosf(__y * __complex_sinhy2);\n"
        + "      float sinhsinh = sinhf(__x * __complex_sinhx1);\n"
        + "      float sinhcosh = coshf(__x * __complex_sinhx2);\n"
        + "      x += __complex_sinhpow * sinhsinh * sinhcos;\n"
        + "      y += __complex_sinhpow * sinhcosh * sinhsin;\n"
        + "    }\n"
        + "    __px += __complex * x;\n"
        + "    __py += __complex * y;\n"
        + "\n"
        + (context.isPreserveZCoordinate() ? "      __pz += __complex * __z;\n" : "");
  }
}
