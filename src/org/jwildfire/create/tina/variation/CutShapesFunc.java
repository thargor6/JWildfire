	 ...
	 @Override
	  public String getGPUCode(FlameTransformationContext context) {
	    return  "		  float x,y;"
	    		...
	    		+"		    }"
	    		+"  "
	    		+"			float2 p=make_float2(x* varpar->cut_shapes_zoom ,y* varpar->cut_shapes_zoom );"
	    		+"			float color;"
	    		+"			float d=0.0f;"
	    		+"			if(varpar->cut_shapes_contour==1)"
	    		+"				color=1.0f;"
	    		+"			else"
	    		+"				color=0.0f;"
	    		+"			"
	    		+"			if( varpar->cut_shapes_type==0)  "
	    		+"			{"
	    		+"				d = cut_shapes_sdHexagram( p, 0.35f );"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==1) "
	    		+"			{"
	    		+"				d = cut_shapes_sdEquilateralTriangle( p );"
	    		+"			}"
	    		+"			else if ( varpar->cut_shapes_type ==2)"
	    		+"			{"
	    		+"				float a=fract( varpar->cut_shapes_time /3.);"
	    		+"				float m = 4.0 + a*a*( varpar->cut_shapes_n *2.0-4.0);  "
	    		+"				d = cut_shapes_sdStar( p, 0.7,varpar->cut_shapes_n,m );"
	    		+"			}"
	    		+"          else if( varpar->cut_shapes_type ==3)"
	    		+"			{"
	    		+"              float2 tmp=make_float2(0.0f,1.57f) + varpar->cut_shapes_time;"
	    		+"				float2 si = cosf(tmp)*0.3f + 0.8f;"
	    		+"				if( si.x<si.y )"
	    		+"				   	si=make_float2(si.y,si.x);"
	    		+"				    "
	    		+"				 float ra = 0.1*sinf( varpar->cut_shapes_time *1.2f);"
	    		+"				 d = cut_shapes_sdCross( p, si, ra );"
	    		+"			}"
	    		+"			else if ( varpar->cut_shapes_type ==4) "
	    		+"			{"
	    		+"				d= cut_shapes_sdOctogon( p, 0.5 );"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==5) "
	    		+"			{"
	    		+"				 d= cut_shapes_sdHexagon( p, 0.5 );"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==6)  "
	    		+"			{"
	    		+"				d= cut_shapes_sdPentagon( p, 0.5 );"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==7)  "
	    		+"			{"
	    		+"				 d = cut_shapes_sdVesica( p, 1.1, 0.8 + 0.2*sin( varpar->cut_shapes_time ) );"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==8)  "
	    		+"			{"
	    		+"			    float ra=0.2+0.15*sinf( varpar->cut_shapes_time *1.3+0.0);"
	    		+"			    float rb=0.2+0.15*sinf( varpar->cut_shapes_time *1.4+0.1);"
	    		+"			    float2  pa = make_float2(-0.6,0.0)+(sinf(make_float2(0.0,2.0)+( varpar->cut_shapes_time *1.1))*(0.4));"
	    		+"			    float2  pb = make_float2(-0.6,0.0)+(sinf(make_float2(1.0,2.5)+( varpar->cut_shapes_time *1.2))*(0.4));"
	    		+"			    float2  pc = make_float2(0.8,0.0);"
	    		+"			    "
	    		+"				d = cut_shapes_sdTrapezoid( p-(pc), ra, rb, 0.5+0.2*sinf(1.3* varpar->cut_shapes_time ) );"
	    		+"			    "
	    		+"			    d = fminf( d, cut_shapes_sdTrapezoid( p, pa, pb , ra, rb ) );"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==9)  "
	    		+"			{"
	    		+"				float ta = 3.14*(0.5+0.5*cos( varpar->cut_shapes_time *0.52+2.0));"
	    		+"				float tb = 3.14*(0.5+0.5*cos( varpar->cut_shapes_time *0.31+2.0));"
	    		+"				float rb = 0.15*(0.5+0.5*cos( varpar->cut_shapes_time *0.41+3.0));"
	    		+"				"
	    		+"				d = cut_shapes_sdArc(p,make_float2(sin(ta),cos(ta)),make_float2(sin(tb),cos(tb)), 0.5, rb);"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==10)  "
	    		+"			{"
	    		+"				float2 ra = cos( make_float2(0.0,1.57)+( varpar->cut_shapes_time )  )*(0.3)+(0.4);"
	    		+"			    d = cut_shapes_sdRhombus( p, ra );"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==11)  "
	    		+"			{"
	    		+"				float2 ra = cos( make_float2(0.0,1.57)+( varpar->cut_shapes_time )  )*(0.3)+(0.4);"
	    		+"			    d = cut_shapes_sdCircle( p, 0.5 );"
	    		+"			}"
	    		+"			else if( varpar->cut_shapes_type ==12)  "
	    		+"			{"
	    		+"				float2 ra = cos( make_float2(0.0,1.57)+( varpar->cut_shapes_time )  )*(0.3)+(0.4);"
	    		...
	    		+"		    	color = mix( color, 1.0, 1.0-smoothstep(0.0,varpar->cut_shapes_thick,fabsf(d)) );"
	    		+"		    }"
	    		+"	    "
	    		+"		    __doHide=false;"
	    		+"		    if( varpar->cut_shapes_invert ==0)"
	    		+"		    {"
	    		+"		      if (color>0.0)"
	    		+"		      { x=0.;"
	    		+"		        y=0.;"
	    		...
	    		+"			      }"
	    		+"		    }"
	    		+"		    __px = varpar->cut_shapes * x;"
	    		+"		    __py = varpar->cut_shapes * y;"
	            + (context.isPreserveZCoordinate() ? "__pz += varpar->cut_shapes * __z;\n" : "");
	  }
	  @Override
