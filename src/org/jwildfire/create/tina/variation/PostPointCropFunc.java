...
public class PostPointCropFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "   short zero = __px == (0+ __post_point_crop_x_offset) && __py == (0+ __post_point_crop_y_offset) && __pz == (0+ __post_point_crop_z_offset);\n"
        + "    short boundaryX = (__px - __post_point_crop_x_offset) <= __post_point_crop_x_width && (__px - __post_point_crop_x_offset) >= -__post_point_crop_x_width;\n"
        + "    short boundaryY = (__py - __post_point_crop_y_offset) <= __post_point_crop_y_width && (__py - __post_point_crop_y_offset) >= -__post_point_crop_y_width;\n"
        + "    short boundaryZ = (__pz - __post_point_crop_z_offset) <= __post_point_crop_z_width && (__pz - __post_point_crop_z_offset) >= -__post_point_crop_z_width && __pz != 0;\n"
        + "    short boundaries;\n"
        + "\n"
        + "    if (__post_point_crop_x_width + __post_point_crop_y_width + __post_point_crop_z_width != 0)\n"
        + "    {\n"
        + "      boundaries = boundaryX || boundaryY || boundaryZ;\n"
        + "    }\n"
        + "    else\n"
        ...
        + "\n"
        + "    if (boundaries) {\n"
        + "      if (__post_point_crop_point_crop_enable == 1)\n"
        + "      {\n"
        ...
        + "      }\n"
        + "      if (__post_point_crop_point_hide_enable == 1)\n"
        + "      {\n"
        + "        __doHide = true;\n"
        + "      }\n"
        + "    }\n"
        ...
        + "      }\n"
        + (context.isPreserveZCoordinate() ? "__pz += __post_point_crop * __z;\n" : "")
        + "    }";
  }
