...
public class CrackleFunc extends VariationFunc implements SupportsGPU {
  ...
  protected static final String[] paramNames = {
    PARAM_CELLSIZE, PARAM_POWER, PARAM_DISTORT, PARAM_SCALE, PARAM_Z
  };
  ...
  public void transform(
    ...
    if (pAmount != 0) {
      ...
      double blurr =
          (pContext.random() + pContext.random()) / 2.0 + (pContext.random() - 0.5) / 4.0;
  ...
  protected void applyCellCalculation(
      XYZPoint pVarTP, double pAmount, double DXo, double DYo, double L) {
  ...
  public void setParameter(String pName, double pValue) {
    ...
    else if (PARAM_DISTORT.equalsIgnoreCase(pName)) distort = pValue;
    else if (PARAM_SCALE.equalsIgnoreCase(pName)) scale = pValue;
    else if (PARAM_Z.equalsIgnoreCase(pName)) z = pValue;
  ...
  private static final int CACHE_WIDTH = 21;
  ...
  // position() calculates cell centre for cell (x, y), given plane slice z, scale factor s,
  // distortion d
  ...
  // cached_position gives centre co-ordinates either from cache, or calculated from scratch if
  // needed
  private void cached_position(
      double Cache[][][], int x, int y, double z, double s, double d, double V[]) {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return
        ...
        + "      U.y = blurr * cosf(theta);\n"
        + "  } else {\n"
        + "    U.x = __x;\n"
        + "    U.y = __y;\n"
        + "  }\n"
        + "    XCv = (int) floorf(U.x / s);\n"
        + "    YCv = (int) floorf(U.y / s);\n"
        + "    int di, dj;\n"
        + "    int i = 0;\n"
        + "    for (di = -1; di < 2; di++) {\n"
        + "      for (dj = -1; dj < 2; dj++) {\n"
        ...
        + "      }\n"
        + "    }\n"
        + "    int q = __state%d_"+getName()+"_closest(P, 9, &U);\n"
        + "\n"
        ...
        + "    DXo += P[4].x;\n"
        + "    DYo += P[4].y;\n"
        + applyCellCalculationGPU(context)
        + "}\n";
  }
  ...
  public String getGPUFunctions(FlameTransformationContext context) {
    return "#define CRACKLE_CACHE_NUM 10\n"
        + "#define CRACKLE_CACHE_WIDTH 21\n"
        + "#define CRACKLE_VORONOI_MAXPOINTS 25\n"
        + "__device__ float2 __state%d_"+getName()+"_C[CRACKLE_CACHE_WIDTH][CRACKLE_CACHE_WIDTH];\n"
        + "__device__ short __state%d_"+getName()+"_cacheInitFlag=0;\n"
        + "\n"
        // using simplexNoise from the base kernel
/*
        + "__constant__ float __state%d_"+getName()+"_grad3[1024][3] = {\n"
        ...
        + "__constant__ int __state%d_"+getName()+"_p[] = {\n"
        ...
        + "__device__ float __state%d_"+getName()+"_simplexNoise3D(float3 *V) {\n"
        + "    float C[4][3];\n"
        + "    float n = 0.0;\n"
        + "    int gi[4];\n"
        + "    int corner;\n"
        + "    float t;\n"
        + "    float skewF3 = 1.0 / 3.0;\n"
        + "    float skewG3 = 1.0 / 6.0;\n"
        + "    float skewIn = (V->x + V->y + V->z) * skewF3;\n"
        + "    int i = (int) floorf(V->x + skewIn);\n"
        + "    int j = (int) floorf(V->y + skewIn);\n"
        + "    int k = (int) floorf(V->z + skewIn);\n"
        + "    t = (float) (i + j + k) * skewG3;\n"
        + "    float X0 = i - t;\n"
        + "    float Y0 = j - t;\n"
        + "    float Z0 = k - t;\n"
        + "    C[0][0] = V->x - X0;\n"
        ...
        + "\n"
        + "    C[1][0] = C[0][0] - i1 + skewG3;\n"
        + "    C[1][1] = C[0][1] - j1 + skewG3;\n"
        + "    C[1][2] = C[0][2] - k1 + skewG3;\n"
        + "    C[2][0] = C[0][0] - i2 + 2.0 * skewG3;\n"
        + "    C[2][1] = C[0][1] - j2 + 2.0 * skewG3;\n"
        + "    C[2][2] = C[0][2] - k2 + 2.0 * skewG3;\n"
        + "    C[3][0] = C[0][0] - 1.0 + 3.0 * skewG3;\n"
        + "    C[3][1] = C[0][1] - 1.0 + 3.0 * skewG3;\n"
        + "    C[3][2] = C[0][2] - 1.0 + 3.0 * skewG3;\n"
        + "    int ii = i & 0x3ff;\n"
        + "    int jj = j & 0x3ff;\n"
        + "    int kk = k & 0x3ff;\n"

        + "    gi[0] = __state%d_"+getName()+"_p[ii + __state%d_"+getName()+"_p[jj + __state%d_"+getName()+"_p[kk]]];\n"
        + "    gi[1] = __state%d_"+getName()+"_p[ii + i1 + __state%d_"+getName()+"_p[jj + j1 + __state%d_"+getName()+"_p[kk + k1]]];\n"
        + "    gi[2] = __state%d_"+getName()+"_p[ii + i2 + __state%d_"+getName()+"_p[jj + j2 + __state%d_"+getName()+"_p[kk + k2]]];\n"
        + "    gi[3] = __state%d_"+getName()+"_p[ii + 1 + __state%d_"+getName()+"_p[jj + 1 + __state%d_"+getName()+"_p[kk + 1]]];\n"

        + "    for (corner = 0; corner < 4; corner++) {\n"
        + "      t = 0.6 - C[corner][0] * C[corner][0] - C[corner][1] * C[corner][1] - C[corner][2] * C[corner][2];\n"
        + "      if (t > 0.0) {\n"
        + "        t *= t;\n"
        + "        n += t * t * (__state%d_"+getName()+"_grad3[gi[corner]][0] * C[corner][0] + __state%d_"+getName()+"_grad3[gi[corner]][1] * C[corner][1] + __state%d_"+getName()+"_grad3[gi[corner]][2] * C[corner][2]);\n"
        + "      }\n"
        + "    }\n"
        + "\n"
        + "    return 32 * n;\n"
...
*/
        + "\n"
        + "__device__ void __state%d_"+getName()+"_position(int x, int y, float z, float s, float d, float2 *V) {\n"
        + "    float3 E;\n"
        + "    float3 F;\n"
        + "    E.x = x * 2.5;\n"
        + "    E.y = y * 2.5;\n"
        ...
        + "    F.z = z * 2.5 + 19.8;\n"
        + "\n"
//        + "    V->x = (x + d * __state%d_"+getName()+"_simplexNoise3D(&E)) * s;\n"
//        + "    V->y = (y + d * __state%d_"+getName()+"_simplexNoise3D(&F)) * s;\n"
       + "    V->x = (x + d * fastNoise::singleSimplex(1337, E.x, E.y, E.z) ) * s;\n"
       + "    V->y = (y + d * fastNoise::singleSimplex(1337, F.x, F.y, F.z) ) * s;\n"
        + "  }\n"
        + "\n"
        + "__device__ int __state%d_"+getName()+"_closest(float2 P[], int n, float2 *U) {\n"
        + "    float d2;\n"
        + "    float d2min = 1.0e32f;\n"
        + "    int j = 0;\n"
        + "    for (int i = 0; i < n; i++) {\n"
        ...
        + "    return j;\n"
        + "  }\n"
        + "__device__ float __state%d_"+getName()+"_vratio(float2 *P, float2 *Q, float2 *U) {\n"
        + "    float PmQx, PmQy;\n"
        + "    PmQx = P->x - Q->x;\n"
        + "    PmQy = P->y - Q->y;\n"
        ...
        + "      return 1.0f;\n"
        + "    }\n"
        + "    return 2.0f * ((U->x - Q->x) * PmQx + (U->y - Q->y) * PmQy) / (PmQx * PmQx + PmQy * PmQy);\n"
        + "  }\n"
        + "\n"
        + "__device__ float __state%d_"+getName()+"_voronoi(float2 P[], int n, int q, float2 *U) {\n"
        + "   float ratio;\n"
        + "   float ratiomax = -1.0e20f;\n"
        + "   for (int i = 0; i < n; i++) {\n"
        + "      if (i != q) {\n"
        + "        ratio = __state%d_"+getName()+"_vratio(&P[i], &P[q], U);\n"
        + "        if (ratio > ratiomax) {\n"
        + "          ratiomax = ratio;\n"
        + "        }\n"
        + "      }\n"
