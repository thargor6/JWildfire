...
public class Spherical3DWFFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    ...
    return "float _regularForm = fabsf(varpar->spherical3D_wf_exponent - 2.f) < 1.e-10f;\n"
        + "bool _dontInvert   = fabsf(varpar->spherical3D_wf_invert) < epsilon;\n"
        + "float r;\n"
        + "if (_regularForm) {\n"
        + "    r = varpar->spherical3D_wf / (__x*__x + __y*__y + __z*__z ADD_EPSILON);\n"
        + "}\n"
        + "else {\n"
        + "    r = varpar->spherical3D_wf /\n"
        + "        powf(__x*__x + __y*__y + __z*__z +ADD_EPSILON, varpar->spherical3D_wf_exponent/2.f);\n"
        + "}\n"
        + "if (_dontInvert) {\n"
        + "    __px += __x * r;\n"
        + "    __py += __y * r;\n"
