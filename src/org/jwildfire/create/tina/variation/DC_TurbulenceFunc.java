	 ...
	 @Override
	  public String getGPUCode(FlameTransformationContext context) {
	    return   "float x,y;"
	    		...
	    		+"float z=0.5;"
	    		+"if( __dc_turbulence_ColorOnly ==1)"
	    		+"{"
	    		+"  x=__x;"
	    		+"  y=__y;"
	    		+"}"
	    		...
	    		+"  y=RANDFLOAT()-0.5;"
	    		+"}"
	    		+"float2 uv=make_float2(x,y)*__dc_turbulence_zoom-20.0;"
	    		+"color=dc_turbulence_getRGBColor(uv,__dc_turbulence_time,__dc_turbulence_zoom);"
	    		+"if( __dc_turbulence_Gradient ==0 )"
	    		+"{"
	    		+"   __useRgb  = true;"
	    		+"   __colorR  = color.x;"
	    		+"   __colorG  = color.y;"
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( __dc_turbulence_Gradient ==1 )"
	    		+"{"
...
////use nearest palette color as the pixel color                
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( __dc_turbulence_Gradient ==2 )"
	    		+"{"
	    		+"  int3 icolor=dbl2int(color);"
	    		+"  float z=greyscale((float)icolor.x,(float)icolor.y,(float)icolor.z);"
	    		+"  __pal=z;"
	    		+"}"
	    		+"__px+= __dc_turbulence*x;"
	    		+"__py+= __dc_turbulence*y;"
	    		+"float dz = z * __dc_turbulence_scale_z + __dc_turbulence_offset_z;"
	    		+"if ( __dc_turbulence_reset_z  == 1) {"
	    		+"     __pz = dz;"
	    		+"}"
	    		+"else {"
	    		+"   __pz += dz;"
