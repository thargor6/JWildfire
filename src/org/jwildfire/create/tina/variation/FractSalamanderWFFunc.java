...
public class FractSalamanderWFFunc extends AbstractFractWFFunc implements SupportsGPU {
  ...
  public VariationFuncType[] getVariationTypes() {
    return new VariationFuncType[]{VariationFuncType.VARTYPE_3D, VariationFuncType.VARTYPE_SIMULATION, VariationFuncType.VARTYPE_DC, VariationFuncType.VARTYPE_BASE_SHAPE, VariationFuncType.VARTYPE_ESCAPE_TIME_FRACTAL, VariationFuncType.VARTYPE_SUPPORTS_GPU, VariationFuncType.VARTYPE_SUPPORTS_BACKGROUND};
  }

  @Override
  public String getGPUCode(FlameTransformationContext context) {
    return "if (__fract_salamander_wf_buddhabrot_mode > 0) {\n"
            + "    float x0=0, y0 = 0;\n"
            + "    int bb_max_clip_iter = 250;\n"
            + "    if (varpar->fract_salamander_wf_chooseNewPoint) {\n"
            + "      for (int i = 0; i < bb_max_clip_iter; i++) {\n"
            + "        x0 = (__fract_salamander_wf_xmax - __fract_salamander_wf_xmin) * RANDFLOAT() + __fract_salamander_wf_xmin;\n"
            + "        y0 = (__fract_salamander_wf_ymax - __fract_salamander_wf_ymin) * RANDFLOAT() + __fract_salamander_wf_ymin;\n"
            + "        if (fract_salamander_wf_preBuddhaIterate(varpar, x0, y0, __fract_salamander_wf_max_iter)) {\n"
            + "          break;\n"
            + "        }\n"
            ...
            + "        for (int skip = 0; skip < __fract_salamander_wf_buddhabrot_min_iter; skip++) {\n"
            ...
            + "      }\n"
            + "      __px += __fract_salamander_wf_scale * __fract_salamander_wf * (varpar->fract_salamander_wf_currX + __fract_salamander_wf_offsetx);\n"
            + "      __py += __fract_salamander_wf_scale * __fract_salamander_wf * (varpar->fract_salamander_wf_currY + __fract_salamander_wf_offsety);\n"
            ...
            + "    float x0 = 0.0, y0 = 0.0;\n"
            + "    int iterCount = 0;\n"
            + "    for (int i = 0; i < __fract_salamander_wf_max_clip_iter; i++) {\n"
            + "      x0 = (__fract_salamander_wf_xmax - __fract_salamander_wf_xmin) * RANDFLOAT() + __fract_salamander_wf_xmin;\n"
            + "      y0 = (__fract_salamander_wf_ymax - __fract_salamander_wf_ymin) * RANDFLOAT() + __fract_salamander_wf_ymin;\n"
            + "      iterCount = fract_salamander_wf_iterate(varpar, x0, y0, __fract_salamander_wf_max_iter);\n"
            + "      if ((__fract_salamander_wf_clip_iter_max < 0 && iterCount >= (__fract_salamander_wf_max_iter + __fract_salamander_wf_clip_iter_max)) || (__fract_salamander_wf_clip_iter_min > 0 && iterCount <= __fract_salamander_wf_clip_iter_min)) {\n"
            + "        if (i == __fract_salamander_wf_max_clip_iter - 1) {\n"
            + "          __doHide = true;\n"
            + "          break;\n"
            + "        }\n"
            + "      } else {\n"
            ...
            + "    }\n"
            + "    if(!__doHide) {\n"
            + "      __px += __fract_salamander_wf_scale * __fract_salamander_wf * (x0 + __fract_salamander_wf_offsetx);\n"
            + "      __py += __fract_salamander_wf_scale * __fract_salamander_wf * (y0 + __fract_salamander_wf_offsety);\n"
            + "      float z;\n"
            + "      if (lroundf(__fract_salamander_wf_z_logscale) == 1) {\n"
            + "        z = __fract_salamander_wf_scale * __fract_salamander_wf * (__fract_salamander_wf_scalez / 10 * log10f(1.0 + (float) iterCount / (float) __fract_salamander_wf_max_iter) + __fract_salamander_wf_offsetz);\n"
            + "        if (__fract_salamander_wf_z_fill > 1.e-6f && RANDFLOAT() < __fract_salamander_wf_z_fill) {\n"
            + "          float prevZ = __fract_salamander_wf_scale * __fract_salamander_wf * (__fract_salamander_wf_scalez / 10 * log10f(1.0 + (float) (iterCount - 1) / (float) __fract_salamander_wf_max_iter) + __fract_salamander_wf_offsetz);\n"
            + "          z = (prevZ - z) * RANDFLOAT() + z;\n"
            + "        }\n"
            + "      } else {\n"
            + "        z = __fract_salamander_wf_scale * __fract_salamander_wf * (__fract_salamander_wf_scalez / 10 * ((float) iterCount / (float) __fract_salamander_wf_max_iter) + __fract_salamander_wf_offsetz);\n"
            + "        if (__fract_salamander_wf_z_fill > 1.e-6f && RANDFLOAT() < __fract_salamander_wf_z_fill) {\n"
            + "          float prevZ = __fract_salamander_wf_scale * __fract_salamander_wf * (__fract_salamander_wf_scalez / 10 * ((float) (iterCount - 1) / (float) __fract_salamander_wf_max_iter) + __fract_salamander_wf_offsetz);\n"
            + "          z = (prevZ - z) * RANDFLOAT() + z;\n"
            + "        }\n"
            + "      }\n"
            + "     __pz += z;\n"
            + "      if (lroundf(__fract_salamander_wf_direct_color) != 0) {\n"
            + "        __pal += (float) iterCount / (float) __fract_salamander_wf_max_iter;\n"
            + "        if (__pal > 1.0)\n"
            + "          __pal -= 1.0;\n"
            + "        if (__pal < 0)\n"
            + "          __pal = 0;\n"
