...
public class ParallelFunc extends VariationFunc implements SupportsGPU {
    ...
    return "float _xr1 = __parallel_x1mod2 * __parallel_x1mod1;\n"
        + "float _xr2 = __parallel_x2mod2 * __parallel_x2mod1;\n"
        + "if (RANDFLOAT() < 0.5f) {\n"
        + "   float x1 = -__parallel_x1width;\n"
        + "   if (RANDFLOAT() < 0.5)\n"
        + "     x1 = __parallel_x1width;\n"
        + "   __px += __parallel_x1tilesize * (__x + roundf(x1 * logf(RANDFLOAT())));\n"
        + "   if (__y > __parallel_x1mod1) {\n"
        + "     __py += __parallel_x1height * (-__parallel_x1mod1 + fmodf(__y + __parallel_x1mod1, _xr1)) + __parallel * __parallel_x1move;\n"
        + "   } else if (__y < -__parallel_x1mod1) {\n"
        + "     __py += __parallel_x1height * (__parallel_x1mod1 - fmodf(__parallel_x1mod1 - __y, _xr1)) + __parallel * __parallel_x1move;\n"
        + "   } else {\n"
        + "    __py += __parallel_x1height * __y + __parallel * __parallel_x1move;\n"
        + "   }\n"
        + "} else {\n"
        + "   float x2 = -__parallel_x2width;\n"
        + "   if (RANDFLOAT() < 0.5)\n"
        + "     x2 = __parallel_x2width;\n"
        + "   __px += __parallel_x2tilesize * (__x + roundf(x2 * logf(RANDFLOAT())));\n"
        + "   if (__y > __parallel_x2mod1) {\n"
        + "     __py += __parallel_x2height * (-__parallel_x2mod1 + fmodf(__y + __parallel_x2mod1, _xr2)) - __parallel * __parallel_x2move;\n"
        + "   } else if (__y < -__parallel_x2mod1) {\n"
        + "     __py += __parallel_x2height * (__parallel_x2mod1 - fmodf(__parallel_x2mod1 - __y, _xr2)) - __parallel * __parallel_x2move;\n"
        + "   } else {\n"
        + "    __py += __parallel_x2height * __y - __parallel * __parallel_x2move;\n"
        + "   }\n"
        + "}\n"
				+ (context.isPreserveZCoordinate() ? "__pz += __parallel * __z;\n" : "");
	}
}
