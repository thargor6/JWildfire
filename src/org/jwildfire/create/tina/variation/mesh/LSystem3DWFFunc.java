 ...
 */ 		
  ...
  // Read input file 
         try
         {
  				...
  				if (keyword.equals("recursion"))
  				{ // Use keyword to determine action
   ...
   // Echo input 
  		...
  		System.out.println("Axiom              : " + object_s);
  		 ...
  		 // Add default rules 
  			...
  			rule.add("_=_");num++; // Closer default
 ...
 // calculate size of each rule         
  	 ...
  	 // Each char gets a rule number 
  	  ...
  	  public  float vector_length(float[] Vector)
  	    {
  	    	return (float) Math.sqrt (Vector[_x]*Vector[_x]+Vector[_y]*Vector[_y]+Vector[_z]*Vector[_z]);
  	    }
  	   ...
  	   public  void Vector_product(float[] A,float[] B,float[]C) 
  	   {
  		...
  		public  void Set_rot(float a, float[] n)
  		...
  		/* Insert basic block. Here we
  		 ...
  		 /* Setup */
  			...
  			d = ((float) Math.sqrt((float)(dis[_x] * dis[_x]) + (float)(dis[_y] * dis[_y]) + (float)(dis[_z] * dis[_z])));
  			if (d == (float) 0.0)
  			{
  				return;
  			}
...
//  			memcpy(((Object)(d1)), ((Object)(dis)), 12);
  			...
  			{
  				float Dist = (float) 1.0 / ((float) Math.sqrt((float)(d1[_x] * d1[_x]) + (float)(d1[_y] * d1[_y]) + (float)(d1[_z] * d1[_z])));
  				d1[_x] *= Dist;
  				d1[_y] *= Dist;
  				d1[_z] *= Dist;
  			};
...
//  			memcpy(((Object)(d2)), ((Object)(up)), 12);
  			...
  			{
  				float Dist = (float) 1.0 / ((float) Math.sqrt((float)(d2[_x] * d2[_x]) + (float)(d2[_y] * d2[_y]) + (float)(d2[_z] * d2[_z])));
  				d2[_x] *= Dist;
  				d2[_y] *= Dist;
  				d2[_z] *= Dist;
  			};
  		...
  		{
  				float Dist = (float) 1.0 / ((float) Math.sqrt((float)(d3[_x] * d3[_x]) + (float)(d3[_y] * d3[_y]) + (float)(d3[_z] * d3[_z])));
  				d3[_x] *= Dist;
  				d3[_y] *= Dist;
  				d3[_z] *= Dist;
  		};
...
//  			memcpy(((Object)(d2)), ((Object)(up)), 12);
  		...
  		{
  				float Dist = (float) 1.0 / ((float) Math.sqrt((float)(t1[_x] * t1[_x]) + (float)(t1[_y] * t1[_y]) + (float)(t1[_z] * t1[_z])));
  				t1[_x] *= Dist;
  				t1[_y] *= Dist;
  				t1[_z] *= Dist;
  		};
  		...
  		{
  				float Dist = (float) 1.0 / ((float) Math.sqrt((float)(t2[_x] * t2[_x]) + (float)(t2[_y] * t2[_y]) + (float)(t2[_z] * t2[_z])));
  				t2[_x] *= Dist;
  				t2[_y] *= Dist;
  				t2[_z] *= Dist;
  		};
