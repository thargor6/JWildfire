...
public class SquarizeFunc extends SimpleVariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "    float s = sqrtf(__x*__x + __y*__y);\n"
        ...
        + "\n"
        + "    if (p <= 1.0 * s) {\n"
        + "      __px += varpar->squarize * s;\n"
        + "      __py += varpar->squarize * p;\n"
        + "    } else if (p <= 3.0 * s) {\n"
        + "      __px += varpar->squarize * (2.0 * s - p);\n"
        + "      __py += varpar->squarize * (s);\n"
        + "    } else if (p <= 5.0 * s) {\n"
        + "      __px -= varpar->squarize * (s);\n"
        + "      __py += varpar->squarize * (4.0 * s - p);\n"
        + "    } else if (p <= 7.0 * s) {\n"
        + "      __px -= varpar->squarize * (6.0 * s - p);\n"
        + "      __py -= varpar->squarize * (s);\n"
        + "    } else {\n"
        + "      __px += varpar->squarize * (s);\n"
        + "      __py -= varpar->squarize * (8.0 * s - p);\n"
        + "    }"
        + (context.isPreserveZCoordinate() ? "__pz += varpar->squarize*__z;\n" : "");
  }
}
