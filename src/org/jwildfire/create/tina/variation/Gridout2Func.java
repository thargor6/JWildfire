...
public class Gridout2Func extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return " float x = rintf(__x) * __gridout2_c;\n"
        + "  float y = rintf(__y) * __gridout2_d;\n"
        + "    if (y <= 0.0) {\n"
        + "      if (x > 0.0) {\n"
        + "        if (-y >= x) {\n"
        + "          __px += __gridout2 * (__x + __gridout2_a);\n"
        + "          __py += __gridout2 * __y;\n"
        + "        } else {\n"
        + "          __px += __gridout2 * __x;\n"
        + "          __py += __gridout2 * (__y + __gridout2_b);\n"
        + "        }\n"
        + "      } else {\n"
        + "        if (y <= x) {\n"
        + "          __px += __gridout2 * (__x + __gridout2_a);\n"
        ...
        + "      }\n"
        + "   }\n"
        + (context.isPreserveZCoordinate() ? "__pz += __gridout2 * __z;\n" : "");
  }
}
