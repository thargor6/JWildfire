...
public class TruchetHexCropFunc extends VariationFunc implements SupportsGPU {
	...
	public VariationFuncType[] getVariationTypes() {
		return new VariationFuncType[]{VariationFuncType.VARTYPE_2D, VariationFuncType.VARTYPE_CROP, VariationFuncType.VARTYPE_SIMULATION,VariationFuncType.VARTYPE_SUPPORTS_GPU};
	}
	  @Override
	  public String getGPUCode(FlameTransformationContext context) {
	    return   "	float M_SQRT3_2 = 0.86602540378443864676372317075249;"
	    		+"	float M_SQRT3 = 1.7320508075688772935274463415059;"
	    		+"		float x = 0.57735026918962576450914878050196 * __x - __y / 3.0; "
	    		+"		float z = 2.0 * __y / 3.0;"
	    		+"		float y = -x - z;"
	    		+"		"
	    		+"		float rx = roundf(x);"
	    		...
	    		+"		float FY = __y - FY_h;"
	    		+"		float add = 0.0;"
	    		+"		if ( __truchet_hex_crop_seed  == 1) {"
	    		+"			if (( ((int)rx % 2) == 0) && (((int)rz % 2) == 0)) { "
	    		+"				add = 1.0471975511965977461542144610932;"
	    		+"			}"
	    		+"		}"
	    		+"		if ( __truchet_hex_crop_seed  >= 2) {"
	    		+"			float hash_f = sinf(FX_h * 12.9898 + FY_h * 78.233 + ( __truchet_hex_crop_seed )) * 43758.5453; "
	    		+"			hash_f = hash_f - floorf(hash_f);"
	    		+"			if (hash_f < 0.5) {"
	    		+"				add = 1.0471975511965977461542144610932; "
	    		+"			}"
	    		+"		}"
	    		+"		float angle = atan2f(FY, FX) + 0.52359877559829887307710723054658 - add; "
	    		+"		float coef = 0.47746482927568600730665129011754; "
	    		+"		float angle2 = floorf(angle * coef) / coef + 0.52359877559829887307710723054658 + add; "
	    		+"		float x0 = cosf(angle2);"
	    		+"		float y0 = sinf(angle2);"
	    		+"		float dist = sqrtf(sqrf(FX - x0) + sqrf(FY - y0));"
	    		+"		float d1 = 0.5 +  __truchet_hex_crop_wd ;"
	    		+"		float d2 = 0.5 -  __truchet_hex_crop_wd ;"
	    		+"		if ( __truchet_hex_crop_inv  == 1) { "
	    		+"			if ((dist > d1) || (dist < d2)) { "
	    		+"				if ( __truchet_hex_crop_mode  < 0.5) {"
	    		+"					FX = 0.0;"
	    		+"					FY = 0.0;"
	    		+"				} else {"
	    		+"					if ( __truchet_hex_crop_mode  < 1.5) {"
	    		+"						FX = x0;"
	    		+"						FY = y0;"
	    		+"					} else {"
	    		+"						float rangle = atan2f(FY - y0, FX - x0);"
	    		+"						float D = 0; "
	    		+"						if (RANDFLOAT() < 0.5) {"
	    		+"							D = d1;"
	    		+"						} else {"
	    		...
	    		+"			}"
	    		+"		}"
	    		+"		__px += (FX + FX_h) * __truchet_hex_crop;"
	    		+"		__py += (FY + FY_h) * __truchet_hex_crop;";
	  }
}
