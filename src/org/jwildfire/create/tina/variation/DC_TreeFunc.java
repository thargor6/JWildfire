	 ...
	 @Override
	  public String getGPUCode(FlameTransformationContext context) {
	    return   "float x,y;"
	    		...
	    		+"  y=RANDFLOAT()-0.5;"
	    		+"}"
	    		+"float2 uv=make_float2(x,y)*varpar->dc_tree_zoom;"
	    		+"color=dc_tree_getRGBColor(uv,varpar->dc_tree_time,varpar->dc_tree_shift,varpar->dc_tree_levels,varpar->dc_tree_style,varpar->dc_tree_thicknes);"
	    		+"if( varpar->dc_tree_Gradient ==0 )"
	    		+"{"
	    		+"   __useRgb  = true;"
	    		+"   __colorR  = color.x;"
	    		+"   __colorG  = color.y;"
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( varpar->dc_tree_Gradient ==1 )"  
	    		+"{"
...
////use nearest palette color as the pixel color                
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( varpar->dc_tree_Gradient ==2 )"
	    		+"{"
	    		+"  int3 icolor=dbl2int(color);"
	    		+"  float z=greyscale((float)icolor.x,(float)icolor.y,(float)icolor.z);"
	    		+"  __pal=z;"
	    		+"}"
	    		+"__px+= varpar->dc_tree*x;"
	    		+"__py+= varpar->dc_tree*y;"
	    		+"float dz = z * varpar->dc_tree_scale_z + varpar->dc_tree_offset_z;"
	    		+"if ( varpar->dc_tree_reset_z  == 1) {"
	    		+"     __pz = dz;"
	    		+"}"
	    		+"else {"
	    		+"   __pz += dz;"
