...
public class DC_CirclesBlueFunc  extends DC_BaseFunc implements SupportsGPU {
	...
	public String getGPUCode(FlameTransformationContext context) {
	    return   "float x,y;"
	    		...
	    		+"float z=0.5;"
	    		+"if( __dc_circlesblue_ColorOnly ==1)"
	    		+"{"
	    		+"  x=__x;"
	    		+"  y=__y;"
	    		+"}"
	    		...
	    		+"  y=2.0*RANDFLOAT()-1.0;"
	    		+"}"
	    		+"float2 uv=make_float2(x,y)*__dc_circlesblue_zoom;"
	    		+"color=dc_circlesblue_getRGBColor(uv,__dc_circlesblue_time,__dc_circlesblue_bubles,__dc_circlesblue_radius);"
	    		+"if( __dc_circlesblue_Gradient ==0 )"
	    		+"{"
	    		+"   __useRgb  = true;"
	    		+"   __colorR  = color.x;"
	    		+"   __colorG  = color.y;"
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( __dc_circlesblue_Gradient ==1 )"
	    		+"{"
...
//// use nearest palette color as the pixel color                
	    		...
	    		+"   __colorA  = 1.0;"
	    		+"}"
	    		+"else if( __dc_circlesblue_Gradient ==2 )"
	    		+"{"
	    		+"  int3 icolor=dbl2int(color);"
	    		+"  float z=greyscale((float)icolor.x,(float)icolor.y,(float)icolor.z);"
	    		+"  __pal=z;"
	    		+"}"
	    		+"__px+= __dc_circlesblue*x;"
	    		+"__py+= __dc_circlesblue*y;"
	    		+"float dz = z * __dc_circlesblue_scale_z + __dc_circlesblue_offset_z;"
	    		+"if ( __dc_circlesblue_reset_z  == 1) {"
	    		+"     __pz = dz;"
	    		+"}"
	    		+"else {"
	    		+"   __pz += dz;"
