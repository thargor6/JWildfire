	    	...
	    	private ParameterModifier modify;
	    				...
	    				SymbolString rightContext)
	    		{
	    	        ...
	    	        if(conditionEval==null)
	    	        	evaluate = new ConditionEvaluator(){ public boolean conditionEval(int i, SymbolString ms){ return true;} };
	    	       else
	...
	//						 sub_line=theString.substring(i+1,theString.length());
					...
					case '@':
	                    ...
	                    if(matcher.find())
	                    {
	                      ...
	                      if(strComb.equals("Q"))
	                      	  setStep(render_step*Math.sqrt(vScale));
	                      if(strComb.equals("I"))
	                      	  setStep(render_step*1.0/vScale);
	                      if( strComb.equals("IQ"))
	                    	  setStep(render_step*1.0/Math.sqrt(vScale));
	                      if(strComb.equals("QI"))
	                     	  setStep(render_step*Math.sqrt(vScale)*1.0/vScale);
                          i=i+matcher.end()+1;
	                    }
						break;
					case '<':
	 ...
	 public DoublePoint2D plotLine(double x1, double y1, double x2, double y2)
	  {
	  double ydiff = y2 - y1;
	  double xdiff = x2 - x1;
	  double m;
	  ...
	  double xoffset=0, yoffset=0;
	    ...
	    if (x2 < x1) { xoffset = -1 * xoffset; }  
	    yoffset = Math.abs(m * xoffset);
	    if (y2 < y1) { yoffset = -1 * yoffset; }
	    if (line_thickness != 0) {
	      xoffset += ((Math.random() - 0.5) * line_thickness);
	      yoffset += ((Math.random() - 0.5) * line_thickness);
	    }
	    xout = x1 + xoffset;
	    yout = y1 + yoffset; 
	  ...
	  public DoublePoint2D plotPoint(double x1, double y1)
	  {
	  double xout = 0, yout = 0;
	  double xoffset=0, yoffset=0;
	  // draw endpoints
	  if (point_thickness != 0) {
	    double roffset = Math.random() * point_thickness;
	    double rangle = (Math.random() * M_2PI);
