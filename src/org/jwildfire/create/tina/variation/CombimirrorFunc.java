...
public class CombimirrorFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "   Complex z;\n"
        ...
        + "    Complex_Scale(&z, varpar->combimirror);\n"
        + "    Complex_Scale(&z2, varpar->combimirror);\n"
        + "    __px = z.re;\n"
        + "    __py = z.im;\n"
        + "    __pz = z2.re;\n"
        + "    if (RANDFLOAT() > varpar->combimirror_pmirror / 2) {\n"
        + "      __px = -__px + varpar->combimirror_pmovex;\n"
        + "      __py = -__py + varpar->combimirror_pmovey;\n"
        + "      __pal = fmodf(__pal + varpar->combimirror_pcolorshift, 1.0);\n"
        + "    }\n"
        + "    if (RANDFLOAT() < varpar->combimirror_vmirror / 2) {\n"
        + "      __px = -__px + varpar->combimirror_vmove;\n"
        + "      __pal = fmodf(__pal + varpar->combimirror_vcolorshift, 1.0);\n"
        + "    }\n"
        + "    if (RANDFLOAT() < varpar->combimirror_hmirror / 2) {\n"
        + "      __py = -__py + varpar->combimirror_hmove;\n"
        + "      __pal = fmodf(__pal + varpar->combimirror_hcolorshift, 1.0);\n"
        + "    }\n"
        + "    if (RANDFLOAT() < varpar->combimirror_zmirror / 2) {\n"
        + "      __pz = -__pz + varpar->combimirror_zmove;\n"
        + "      __pal = fmodf(__pal + varpar->combimirror_zcolorshift, 1.0);\n"
        + "    }\n";
  }
}
