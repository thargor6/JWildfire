...
public class Glitchy1Func extends VariationFunc implements SupportsGPU {
    ...
    return "float h = sinf(__glitchy1_angle / 180.0 * PI);\n"
        + "float k = cosf(__glitchy1_angle / 180.0 * PI);\n"
        + "float x = __x;\n"
        + "float y = __y;\n"
        + "float z = __z;\n"
        + "int nx = lroundf(__glitchy1_n_x);\n"
        + "int ny = lroundf(__glitchy1_n_y);\n"
        + "int nz = lroundf(__glitchy1_n_z);\n"
        + "int seed = lroundf(__glitchy1_seed);\n"
        + "        if (__glitchy1_scale_x != 0.0) {\n"
        + "            int nr = (int) floorf((x - __glitchy1_shift_x) * __glitchy1_scale_x);\n"
        + "            if (nr >= 0) {\n"
        + "                if (nr % nx == (nx - 1))\n"
        + "                    x = 2 * __glitchy1_shift_x - x;\n"
        + "            } else {\n"
        + "                if (nr % nx == 0)\n"
        + "                    x = 2 * __glitchy1_shift_x - x;\n"
        + "            }\n"
        + "        }\n"
        + "        if (__glitchy1_scale_y != 0.0) {\n"
        + "            int nr = (int) floorf((y - __glitchy1_shift_y) * __glitchy1_scale_y);\n"
        + "            if (nr >= 0) {\n"
        + "                if (nr % ny == (ny - 1))\n"
        + "                    y = 2 * __glitchy1_shift_y - y;\n"
        + "            } else {\n"
        + "                if (nr % ny == 0)\n"
        + "                    y = 2 * __glitchy1_shift_y - y;\n"
        + "            }\n"
        + "        }\n"
        + "        if (__glitchy1_scale_z != 0.0) {\n"
        + "            int nr = (int) floorf((z - __glitchy1_shift_z) * __glitchy1_scale_z);\n"
        + "            if (nr >= 0) {\n"
        + "                if (nr % nz == (nz - 1))\n"
        + "                    z = 2 * __glitchy1_shift_z - z;\n"
        + "            } else {\n"
        + "                if (nr % nz == 0)\n"
        + "                    z = 2 * __glitchy1_shift_z - z;\n"
        + "            }\n"
        + "        }\n"
        + " \n"
        + "        float uu = __glitchy1_u * __x;\n"
        + "        float vv = __glitchy1_v * (k * uu + h * __y);\n"
        + "        float ww = __glitchy1_w * (k * uu - h * __y);\n"
        + "        int blocku = (int) floorf(uu * __glitchy1_width);\n"
        + "        blocku += (2.0 - 4.0 * glitchy1_hash(blocku * seed + 1));\n"
        + "        int blockv = (int) floorf(vv * __glitchy1_width);\n"
        + "        blockv += (2.0 - 4.0 * glitchy1_hash(blockv * seed + 1));\n"
        + "        int blockw = (int) floorf(ww * __glitchy1_width);\n"
        ...
        + "        __px += __glitchy1 * x * fLen;\n"
        + "        __py += __glitchy1 * y * fLen;\n"
        + "        __pz += __glitchy1 * z * fLen;\n";
    }
