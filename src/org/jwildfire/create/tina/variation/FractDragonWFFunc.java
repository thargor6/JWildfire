...
public class FractDragonWFFunc extends AbstractFractWFFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "if (varpar->fract_dragon_wf_buddhabrot_mode > 0) {\n"
            + "    float x0=0, y0 = 0;\n"
            + "    int bb_max_clip_iter = 250;\n"
            + "    if (varpar->fract_dragon_wf_chooseNewPoint) {\n"
            + "      for (int i = 0; i < bb_max_clip_iter; i++) {\n"
            + "        x0 = (varpar->fract_dragon_wf_xmax - varpar->fract_dragon_wf_xmin) * RANDFLOAT() + varpar->fract_dragon_wf_xmin;\n"
            + "        y0 = (varpar->fract_dragon_wf_ymax - varpar->fract_dragon_wf_ymin) * RANDFLOAT() + varpar->fract_dragon_wf_ymin;\n"
            + "        if (fract_dragon_wf_preBuddhaIterate(varpar, x0, y0, varpar->fract_dragon_wf_max_iter)) {\n"
            + "          break;\n"
            + "        }\n"
            ...
            + "        for (int skip = 0; skip < varpar->fract_dragon_wf_buddhabrot_min_iter; skip++) {\n"
            ...
            + "      }\n"
            + "      __px += varpar->fract_dragon_wf_scale * varpar->fract_dragon_wf * (varpar->fract_dragon_wf_currX + varpar->fract_dragon_wf_offsetx);\n"
            + "      __py += varpar->fract_dragon_wf_scale * varpar->fract_dragon_wf * (varpar->fract_dragon_wf_currY + varpar->fract_dragon_wf_offsety);\n"
            ...
            + "    float x0 = 0.0, y0 = 0.0;\n"
            + "    int iterCount = 0;\n"
            + "    for (int i = 0; i < varpar->fract_dragon_wf_max_clip_iter; i++) {\n"
            + "      x0 = (varpar->fract_dragon_wf_xmax - varpar->fract_dragon_wf_xmin) * RANDFLOAT() + varpar->fract_dragon_wf_xmin;\n"
            + "      y0 = (varpar->fract_dragon_wf_ymax - varpar->fract_dragon_wf_ymin) * RANDFLOAT() + varpar->fract_dragon_wf_ymin;\n"
            + "      iterCount = fract_dragon_wf_iterate(varpar, x0, y0, varpar->fract_dragon_wf_max_iter);\n"
            + "      if ((varpar->fract_dragon_wf_clip_iter_max < 0 && iterCount >= (varpar->fract_dragon_wf_max_iter + varpar->fract_dragon_wf_clip_iter_max)) || (varpar->fract_dragon_wf_clip_iter_min > 0 && iterCount <= varpar->fract_dragon_wf_clip_iter_min)) {\n"
            + "        if (i == varpar->fract_dragon_wf_max_clip_iter - 1) {\n"
            + "          __doHide = true;\n"
            + "          break;\n"
            + "        }\n"
            + "      } else {\n"
            ...
            + "    }\n"
            + "    if(!__doHide) {\n"
            + "      __px += varpar->fract_dragon_wf_scale * varpar->fract_dragon_wf * (x0 + varpar->fract_dragon_wf_offsetx);\n"
            + "      __py += varpar->fract_dragon_wf_scale * varpar->fract_dragon_wf * (y0 + varpar->fract_dragon_wf_offsety);\n"
            + "      float z;\n"
            + "      if (lroundf(varpar->fract_dragon_wf_z_logscale) == 1) {\n"
            + "        z = varpar->fract_dragon_wf_scale * varpar->fract_dragon_wf * (varpar->fract_dragon_wf_scalez / 10 * log10f(1.0 + (float) iterCount / (float) varpar->fract_dragon_wf_max_iter) + varpar->fract_dragon_wf_offsetz);\n"
            + "        if (varpar->fract_dragon_wf_z_fill > 1.e-6f && RANDFLOAT() < varpar->fract_dragon_wf_z_fill) {\n"
            + "          float prevZ = varpar->fract_dragon_wf_scale * varpar->fract_dragon_wf * (varpar->fract_dragon_wf_scalez / 10 * log10f(1.0 + (float) (iterCount - 1) / (float) varpar->fract_dragon_wf_max_iter) + varpar->fract_dragon_wf_offsetz);\n"
            + "          z = (prevZ - z) * RANDFLOAT() + z;\n"
            + "        }\n"
            + "      } else {\n"
            + "        z = varpar->fract_dragon_wf_scale * varpar->fract_dragon_wf * (varpar->fract_dragon_wf_scalez / 10 * ((float) iterCount / (float) varpar->fract_dragon_wf_max_iter) + varpar->fract_dragon_wf_offsetz);\n"
            + "        if (varpar->fract_dragon_wf_z_fill > 1.e-6f && RANDFLOAT() < varpar->fract_dragon_wf_z_fill) {\n"
            + "          float prevZ = varpar->fract_dragon_wf_scale * varpar->fract_dragon_wf * (varpar->fract_dragon_wf_scalez / 10 * ((float) (iterCount - 1) / (float) varpar->fract_dragon_wf_max_iter) + varpar->fract_dragon_wf_offsetz);\n"
            + "          z = (prevZ - z) * RANDFLOAT() + z;\n"
            + "        }\n"
            + "      }\n"
            + "     __pz += z;\n"
            + "      if (lroundf(varpar->fract_dragon_wf_direct_color) != 0) {\n"
            + "        __pal += (float) iterCount / (float) varpar->fract_dragon_wf_max_iter;\n"
            + "        if (__pal > 1.0)\n"
            + "          __pal -= 1.0;\n"
            + "        if (__pal < 0)\n"
            + "          __pal = 0;\n"
