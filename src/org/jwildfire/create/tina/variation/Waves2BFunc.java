...
public class Waves2BFunc extends VariationFunc implements SupportsGPU {
  ...
  public String getGPUCode(FlameTransformationContext context) {
    return "float _six, _siy;\n"
        + "_six = __waves2b_scalex - __waves2b_scaleinfx;\n"
        + "_siy = __waves2b_scaley - __waves2b_scaleinfy;\n"
        + "float CsX = 1.0f;\n"
        + "float CsY = 1.0f;\n"
        + "float JCB_sn;\n"
        + "CsX = waves2b_safediv(__waves2b_unity, (__waves2b_unity + __x*__x));\n"
        + "CsX = CsX * _six + __waves2b_scaleinfx;\n"
        + "CsY = waves2b_safediv(__waves2b_unity, (__waves2b_unity + __y*__y));\n"
        + "CsY = CsY * _siy + __waves2b_scaleinfy;\n"
        + "if (__waves2b_pwx >= 0.f && __waves2b_pwx < 1.0e-4f) {\n"
        + "  JCB_sn = waves2b_Jacobi_elliptic(__y * __waves2b_freqx, __waves2b_jacok);\n"
        + "  __px += __waves2b * (__x + CsX * JCB_sn);\n"
        + "} else if (__waves2b_pwx < 0.f && __waves2b_pwx > -1.0e-4f) {\n"
        + "  __px += __waves2b * (__x + CsX * j1f(__y * __waves2b_freqx));\n"
        + "} else {\n"
        + "  __px += __waves2b * (__x + CsX * sinf((__y<0.f ? -1 : __y>0.f ? 1 : 0) * powf(fabsf(__y) + 1e-10f, __waves2b_pwx) * __waves2b_freqx));\n"
        + "}\n"
        + "if (__waves2b_pwy >= 0.f && __waves2b_pwy < 1.0e-4f) {\n"
        + "   JCB_sn = waves2b_Jacobi_elliptic(__x * __waves2b_freqy, __waves2b_jacok);\n"
        + "   __py += __waves2b * (__y + CsY * JCB_sn);\n"
        + "} else if (__waves2b_pwy < 0.f && __waves2b_pwy > -1.0e-4f) {\n"
        + "   __py += __waves2b * (__y + CsY * j1f(__x * __waves2b_freqy));\n"
        + "} else {\n"
        + "   __py += __waves2b * (__y + CsY * sinf((__x<0.f ? -1 : __x>0.f ? 1 : 0) * powf(fabsf(__x) + 1e-10f, __waves2b_pwy) * __waves2b_freqy));\n"
        + "}\n"
        + (context.isPreserveZCoordinate() ? "__pz += __waves2b * __z;\n" : "");
  }
  @Override
